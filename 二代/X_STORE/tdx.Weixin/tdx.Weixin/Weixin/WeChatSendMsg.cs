...
namespace tdx.Weixin
{
    public class WeChatSendMsg
    {
        ...
        public WeChatSendMsg(string _wxUname,string _wxUpsw,string _wxMsgID,string[] _sendMsgParam)
        {
            wxUname = _wxUname;
            wxUpsw = _wxUpsw;
            wxMsgID = _wxMsgID;
            ...
            { 
                ...
                if (text2.Length > 0)
                {
                    if (text2.Contains("&token="))
                    {
                        ...
                        HttpContext.Current.Session["weixinCookie"] = cc;
                        LoginInfo.LoginCookie = cc;
                        HttpContext.Current.Session["weixincreateDate"] = DateTime.Now;
                        LoginInfo.CreateDate = DateTime.Now; 
                        HttpContext.Current.Session["weixinToken"] = token;
        ...
        //方法：模拟发送
        public string weChatSend(string _targetWeChat)
        {
            string result = "false";
            ...
            try
            {
                ...
                if (ImgandTxt.Count > 0)
                {
                    ...
                    for (int q = 0; q < (ImgandTxt[0]["multi_item"].Count()); q++)
                    {
                        ...
                        pad.Append("&content" + q.ToString() + "=" + string.Format(ct, this.sendMsgParam[0], this.sendMsgParam[1], this.sendMsgParam[2], this.sendMsgParam[3], this.sendMsgParam[4]));//详细内容
                        pad.Append("&digest" + q.ToString() + "=" + string.Format((ImgandTxt[0]["multi_item"])[q]["digest"].ToString(), this.sendMsgParam[0], this.sendMsgParam[1], this.sendMsgParam[2], this.sendMsgParam[3], this.sendMsgParam[4]));//摘要
    ...
    public class WeiXinUrl
    {
        ...
        /// 登陆的URL POST请求
        /// </summary>
        public static string Denlu_Url = "http://mp.weixin.qq.com/cgi-bin/login?lang=zh_CN ";
        /// <summary>
