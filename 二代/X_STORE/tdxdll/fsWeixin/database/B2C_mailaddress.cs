...
{
    public class B2C_mailaddress
    {
         public int id = 0;//编号
        public string mailaddress = "";//名称
        public B2C_mailaddress() { }
        //public B2C_mailaddress(int _id)
        //{
        //    Mvip_id = _id;
        //    this.load();
        //}
        ...
        public static DataTable GetList(string _dzd,string _sql)
        {
            try
            {
                DataTable dt = comfun.GetDataTableBySQL("select " + _dzd + " from B2C_mailaddress where " + _sql + " ");
                return dt;
            }
            catch (Exception ex)
            {
