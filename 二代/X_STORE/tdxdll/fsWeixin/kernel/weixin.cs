...
namespace tdx.kernel
{
    public class weixin
    {
        ...
        private string devlopID = "wxa65c65c2a87aee60";
        private string devlogPsw = "80e682751e5af7c2dc0ee5f23c2b8f44";
        public string AccessToken = "";

        #region"验证及初始化"
        public weixin() { }
        public weixin(string _devlopID, string _devlogPsw)
        {
        ...
        private void ResponseMsg(RequestXML requestXML)
        {
            try
            {
                ...
                {
                    string _sql = "select id,k_answer,fid,k_url,k_image,k_content,k_url2,k_des from wx_keys where (','+k_words+',') like '%," + requestXML.Content + ",%' and k_words=(select top 1 k_words from wx_keys where (','+k_words+',') like '%," + requestXML.Content + ",%' group by k_words order by newID()) order by k_sort,id desc";
        ...
        public string GetNichen(string _toUser,string _appID,string _appPSW)
        {
           ...
           _result = weixinUserHandle(_result).Nickname;
    ...
    public class access_tokenTicket
    {
