...
namespace tdx.database
{
    public class B2C_share
    {
        ...
        public void Update()
        {
            if (this.id != 0)
            {                       // , this.cityID
                this.myUpdateMethod(this.id, this.t_hits, this.t_isactive, this.t_ischead, this.t_title, this.t_msg, this.t_gif, this.t_wdate,this.t_bdate,this.t_edate);
            }
            else
            {                     //this.cityID,
                this.myInsertMethod( this.t_hits, this.t_isactive, this.t_ischead, this.t_title, this.t_msg, this.t_gif, this.t_wdate, this.t_bdate, this.t_edate);
            }
        }                          //, int _cityID
        private void myUpdateMethod(int _id, int _t_hits, int _t_isactive, int _t_ischead, string _t_title, string _t_msg, string _t_gif, DateTime _t_wdate,DateTime _t_bdate,DateTime _t_edate)
        {                            //cityID=@cityID,
            string sql = "update B2C_share set t_hits=@t_hits,t_isactive=@t_isactive,t_ischead=@t_ischead,t_title=@t_title,t_msg=@t_msg,t_gif=@t_gif,t_wdate=@t_wdate,t_bdate=@t_bdate,t_edate=@t_edate  where id=@id";
        ...
        private void myInsertMethod(int _t_hits, int _t_isactive, int _t_ischead, string _t_title, string _t_msg, string _t_gif, DateTime _t_wdate, DateTime _t_bdate, DateTime _t_edate)
        {                                      //cityID,                                                               @cityID,
            string sql = "insert into B2C_share (t_hits,t_isactive,t_ischead,t_title,t_msg,t_gif,t_wdate,t_bdate,t_edate) values (@t_hits,@t_isactive,@t_ischead,@t_title,@t_msg,@t_gif,@t_wdate,@t_bdate,@t_edate)";
            SqlParameter[] paras = new SqlParameter[] { 
                    //new SqlParameter("@cityID", _cityID), 
                    new SqlParameter("@t_hits", _t_hits), 
                    new SqlParameter("@t_isactive", _t_isactive), 
