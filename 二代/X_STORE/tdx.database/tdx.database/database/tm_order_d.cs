...
namespace tdx.database
{
    public class tm_order_d
    {
        ...
        public void load()
        {
            string sql = "select * from tm_order_d where id=" + id + "";
            ...
            if (dt.Rows.Count > 0)
            {
                if (dt.Rows.Count > 1)
                {
                    throw new NotSupportedException("tm_order_d ID：" + id + "不唯一");
                }
                else
                {
            ...
            {
                throw new NotSupportedException("tm_order_d：" + id + ":" + "不存在");
            }
        ...
        private void myInsert(string _ono, double  _od_price,int _od_hits, double  _od_amt, string _od_ip, string  _od_from, DateTime  _od_date,string _od_des)
        {
            try
            {
                string sql = "insert into tm_order_d (ono,od_price,od_hits,od_amt,od_ip,od_from,od_date,od_des) values (@ono,@od_price,@od_hits,@od_amt,@od_ip,@od_from,@od_date,@od_des)";
        ...
        private void myUpdate(int _id, string _ono, double  _od_price, int _od_hits, double  _od_amt, string _od_ip, string _od_from, DateTime  _od_date, string _od_des)
        {
            ...
            try
            {
                string sql = "update tm_order_d set ono=@ono,od_price=@od_price,od_hits=@od_hits,od_amt=@od_amt,od_ip=@od_ip,od_from=@od_from,od_date=@od_date,od_des=@od_des where id=" + _id;
                SqlParameter[] paras = new SqlParameter[] { 
                    new SqlParameter("@ono", _ono), 
                    new SqlParameter("@od_price", _od_price), 
                    new SqlParameter("@od_hits", _od_hits), 
        ...
        public static int myDel(int _id)
        {
            ...
            {
                string sql = "delete from tm_order_d where id=" + _id + "";
                try
                {
        ...
        public static DataTable GetList(string _dzd, string _sql)
        {
            ...
            try
            {
                dt = comfun.GetDataTableBySQL("select " + _dzd + " from tm_order_d where " + _sql + "");
            }
            catch (Exception ex)
            {
                throw ex;
