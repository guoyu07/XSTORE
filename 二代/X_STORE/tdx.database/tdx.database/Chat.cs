...
namespace tdx.database
{
    ...
    public class Chat
    {
        ...
        public string appid()
        {
            ...
            if (wxid == 0)
            {
                sqlgetapp = "select * from wx_mp order by id desc";
            }
            else
            {
        ...
        public string access_token()
        {
            ...
            if (wxid == 0)
            {
                sqlselendtime = "select * from wp_token";
            }
            else
            {
            }                sqlselendtime = "select * from wp_token where id=" + wxid + "";
            ...
            if (dtselendtime.Rows.Count > 0)
            {
                if (nowtime >= Convert.ToInt32(dtselendtime.Rows[0]["endtime"]))
                {
                    ...
                    string updateendtime = "update wp_token set access_token='" + jo2["access_token"].ToString() + "',endtime=" + endtime + " where wxid=" + wxid + "";
                ...
                //string inserttoken = "insert into wp_token (access_token,endtime,wxid) values('" + jo2["access_token"].ToString() + "'," + endtime + "," + wxid + ")";
        ...
        {
            string tokenget = token(appid(), secret());
            JObject jo2 = (JObject)JsonConvert.DeserializeObject(tokenget);
            return jo2["access_token"].ToString();
        ...
        public string GetJsapi_Ticket()
        {
            ...
            string result = func.webRequestGet(url_Jsapi_Ticket);
            //return result;
            jsapi_ticket deserializedProduct = (jsapi_ticket)JsonConvert.DeserializeObject(result, typeof(jsapi_ticket));

            return deserializedProduct.ticket;
        }
