...
function uploadSuccess(file, serverData) {
    try {
        ...
        if (jsonstr.status == '1') {
            ...
            $(this.customSettings.upload_target).siblings("a.fancy").children("img.upload-img").attr("src", jsonstr.thumb);
            $(this.customSettings.upload_target).siblings("a.fancy").attr("href", jsonstr.path);
            // $(this.customSettings.upload_target).siblings(".upload-path").val(jsonstr.path + ',' + jsonstr.thumb);
            $(this.customSettings.upload_target).siblings(".upload-path").val(jsonstr.path);
        }
        if (jsonstr.status == '2') {
            var str = "";
            ...
            if (str != "") {
                ...
                $(this.customSettings.upload_target).siblings("a.fancy").children("img.upload-img").attr("src", jsonstr.list[0]);
                $(this.customSettings.upload_target).siblings("a.fancy").attr("href", jsonstr.list[1]);
                $(this.customSettings.upload_target).siblings(".upload-path").val(str);
            }
        }
    } catch (ex) {
        this.debug(ex);
...
function addImage(targetObj, originalSrc, thumbSrc) {
    var filed = $(targetObj).attr("field");
    switch (filed) {
        case "pics":
            //插入到相册UL里面
            var newLi = $('<li>'
            ...
            + '<div class="img-box">'
            + '<a href="' + originalSrc + '" class="fancy" rel="lightbox-group">'
          ...
          //  + '<br/><span>序号：</span><input type="text" class="normal input" style="width:50px" name="txt_序号"   />'
            + '</li>');
            targetObj.siblings(".photo-list").children("ul").html(newLi);
        ...
        case "pics_2":
            //插入到相册UL里面
            var newLi = $('<li>'
             + '<input type="hidden" name="hid_photo_name2" value="0|' + originalSrc + '|' + thumbSrc + '" />'
            + '<div class="img-box">'
            + '<a href="' + originalSrc + '" class="fancy" rel="lightbox-group">'
            + '<img src="' + thumbSrc + '"></img>'
             + '</a>'
        ...
        case "pics_3":
            //插入到相册UL里面
            var newLi = $('<li>'
             + '<input type="hidden" name="hid_photo_name3" value="0|' + originalSrc + '|' + thumbSrc + '" />'
            + '<div class="img-box">'
            + '<a href="' + originalSrc + '" class="fancy" rel="lightbox-group">'
            + '<img src="' + thumbSrc + '"></img>'
             + '</a>'
        ...
        case "pics_4":
            //插入到相册UL里面
            var newLi = $('<li>'
             + '<input type="hidden" name="hid_photo_name4" value="0|' + originalSrc + '|' + thumbSrc + '" />'
            + '<div class="img-box">'
            + '<a href="' + originalSrc + '" class="fancy" rel="lightbox-group">'
            + '<img src="' + thumbSrc + '"></img>'
             + '</a>'
