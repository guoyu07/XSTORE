...
{
    public partial class B2C_tclass_Add : workAuth
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                lt_friendly.Text = "<span class='tipsTitle'>小提示：</span> 编辑您的产品类别，如：新闻动态、公司新闻、行业资讯等等。";
                try
                {
                    ...
                    if (Request["id"] != null)
                    {
                        ...
                        B2C_tclass tt = new B2C_tclass(id);
                        ...
                        try
                        {
                            B2C_tclass parent_cate = new B2C_tclass(Convert.ToInt32(tt.c_parent));
                            parent_cate = null;
                        }
                        catch (Exception ex)
                        {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            if (Request["parent"] != null)
                            {
                                ...
                                B2C_tclass parent_cate = new B2C_tclass(parentID);
                                parent_cate = null;
                            }
                        }
                        catch (Exception ex)
                ...
                {
                    comfun.ChuliException(ex, "man/Texts/B2C_tclass_Add.cs", Session["wID"].ToString());
                }
            }
        }
        protected void btnSave_ServerClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (_mc == "")
                {
                    lt_result.Text = "图文类别名称不能为空!";
                    return;
                }
                if (_mc.Length > 200)
                {
                    lt_result.Text = "图文类别名称字符数不能超过200!";
                    return;
                }
                if (_ms.Length > 300)
                {
