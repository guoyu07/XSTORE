...
{
    public partial class B2C_Pclass_Add : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Request["id"] != null)
                {
                    ...
                    B2C_Pclass tt = new B2C_Pclass(id);
                    ...
                    try
                    {
                        B2C_Pclass parent_cate = new B2C_Pclass(Convert.ToInt32(tt.c_parent));
                        txtparent.Value = parent_cate.c_name;
                        parent_cate = null;
                    }
                    catch (Exception ex)
                    {
                        txtparent.Value = "没有父类";
                    }
                }
                else
                {
                    ...
                    try
                    {
                        if (Request["parent"] != null)
                        {
                            ...
                            B2C_Pclass parent_cate = new B2C_Pclass(parentID);
                            txtparent.Value = parent_cate.c_name;
                            parent_cate = null;
                        }
                    }
        ...
        protected void btnSave_ServerClick(object sender, EventArgs e)
        {
            ...
            string _gif = comFunction.NoHTML(txtgif.Value);
            ...
            if (Request["id"] != null)
            {
                try
                {
                    if (dt.Rows.Count <= 1)
                    {
                        ...
                        tt.c_des = _ms;
                        tt.Update();
                        Response.Write("<script language='javascript'>alert('修改成功！');location.href='B2C_Pclass_List.aspx';</script>");
                    }
                    else
                    {
            ...
            else
            {
                try
                {
                    if (dt.Rows.Count <= 0)
                    {
                        ...
                        tt.c_des = _ms;
                        tt.Update();
                        Response.Write("<script language='javascript'>alert('添加成功！');location.href='B2C_Pclass_List.aspx';</script>");
                    }
                    else
                    {
                        Response.Write("<script language='javascript'>alert('已经存在相同的类别名称！');history.go(-1);</script>");
