...
{
    public partial class B2C_JiFen_List : workAuth
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                try
                {
                    ...
                    int totalcount = Convert.ToInt32(comfun.GetDataTableBySQL("select count(*) from B2C_Goods where (g_buytype = 2) and g_isdel=0").Rows[0][0]); // cityid=" + Session["wID"].ToString().Trim() + " AND
                    //
                    lt_pagearrow.Text = Creatrue.Common.commonTool.GetArrowHtmlNew(_page, totalcount, 20, Request.Form, Request.QueryString);
                    lb_catelist.Text = ClassList(consts.pagesize_Txt, _page, "  (g_buytype = 2) and g_isdel=0"); // cityid=" + Session["wID"].ToString().Trim() + " AND

                    lb_cateadd.Text = "<input type=\"button\" class=\"btnAdd\" onclick=\"location.href='B2C_JiFen_Add.aspx'\" class=\"btnAdd\" value=\"添加产品内容\" />";
                }
                catch (Exception ex)
                {
                    comfun.ChuliException(ex, "man/Goods/B2C_JiFen_List.cs", Session["wID"].ToString());
                }
            }
        }
        ...
        protected string ClassList(int pagesize, int page, string _sql)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                str += @"          <td >" + Convert.ToDouble(Creatrue.Common.commonTool.GetMoney(dr["g_price_s"].ToString())).ToString("f2") + "/" + Convert.ToDouble(Creatrue.Common.commonTool.GetMoney(dr["g_cent"].ToString())).ToString("f0") + "</td>";
                str += @"          <td >" + dr["regtime"] + "</td>";
                str += @"          <td ><a href='B2C_JiFen_Add.aspx?id=" + dr["id"] + "'><img width=\"20\" height=\"20\" src=\"/man/images4/Icon_xiugai.png\"></a></td>";
                str += @"        </tr>";
            }
            str += @"</table>";
            return str;
        ...
        protected void ss_btn_ServerClick(System.Object sender, System.EventArgs e)
        {
            ...
            try
            {
                ...
                string sql = "  g_name like '%" + keyword + "%' and g_buytype = 2";// cityid=" + Session["wID"].ToString().Trim() + " and
                int _page = (Request["page"] != null ? Convert.ToInt32(Request["page"]) : 1);
