...
{
    public partial class tm_Orders_Detail : workAuth
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (Session["ono"] != null)
                {
                    tm_orders bo = new tm_orders(_ono);
                    ...
                    DataTable dat = tm_orders.GetAddr(_ono);
                    if (dat.Rows.Count > 0)
                    {
        ...
        private string GetOrderDetailList()
        {
            ...
            DataTable dat = tm_orders.GetDetailList(_ono);
            ...
            foreach (DataRow dr in dat.Rows)
            {
                ...
                resultStr += @"          <td align=center><a href='tm_orderD_Mod.aspx?id=" + dr["id"] + "'>修改</a></td>";
                resultStr += @"        </tr>";
            }
            resultStr += @"</table>";
        ...
        private string GetOrderLogList()
        {
            ...
            DataTable dat = tm_orders.GetLogsList(_ono);
        ...
        protected void btnDelete_ServerClick(object sender, EventArgs e)
        {
            ...
            if (Request["delbox"] != null)
            {
                ...
                try
                {
                    tm_orders.deleteDetail(delid);
                    Response.Write("<script language='javascript'>alert('删除成功！');location.href='tm_orders_Detail.aspx?ono=" + Session["ono"].ToString().Trim() + "';</script>");
                }
                catch (Exception)
                {
        ...
        protected void btnFlag_ServerClick(object sender, EventArgs e)
        {
            if (Session["ono"] != null)
            {
                ...
                string _ono = Session["ono"].ToString().Trim();
                string _sql = "";
                _sql += tm_orders.insertLogsSQL(_ono, "管理员", "", Convert.ToInt32(_aid));
                _sql += "update tm_orders set aid=" + _aid + " where o_no='" + _ono + "'";

                try
                {
                    comfun.UpdateBySQL(_sql);
                    Response.Write("<script language='javascript'>alert('更新订单状态成功！');location.href='tm_orders_Detail.aspx?ono=" + _ono + "';</script>");
                }
                catch (Exception)
                {
        ...
        protected void btnFlag2_ServerClick(object sender, EventArgs e)
        {
            if (Session["ono"] != null)
            {
                ...
                string _sql = "";
                _sql += "update tm_orders set o_stid=" + _stid + ",o_st_amt=" + _stAmt + ",o_allamt=o_amt+" + _stAmt + " where o_no='" + _ono + "'";

                try
                {
                    comfun.UpdateBySQL(_sql);
                    Response.Write("<script language='javascript'>alert('更新配送方式成功！');location.href='tm_orders_Detail.aspx?ono=" + _ono + "';</script>");
                }
                catch (Exception)
                {
        ...
        {
            Response.Redirect("tm_orders_Mod.aspx?ono=" + Session["ono"].ToString().Trim());
        }
