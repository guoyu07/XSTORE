...
{
    public partial class ms_order_log_List : workAuth
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                int totalcount = Convert.ToInt32(comfun.GetDataTableBySQL("select count(*) from ms_order_log").Rows[0][0]);
        ...
        protected string ClassList(int pagesize, int page)
        {
            ...
            string isactive = "";
            string isdel = "";
            string sql = "with c as (select row_number() over(order by ol_date desc) as rown, * from ms_order_log) select top " + pagesize.ToString() + " * from c where rown > " + ((page - 1) * pagesize).ToString() + "order by rown";
            DataTable dt = comfun.GetDataTableBySQL(sql);
            string str = "";
            str += @"<table width=100% border=0 align=center cellpadding=0 cellspacing=0  class='borderTable'>>";
            str += @"       <tr>";
            str += "        <td height=\"30\" align=\"center\" class=\"red\"><input type=\"checkbox\" class=\"btn\"  name=\"delAll\" id=\"delAll\" runat=\"server\" onclick =\"this.value=checkAll(form1.delbox,this);\" />全选</td>";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //str += @"          <td align=center><a href='B2C_ms_Add.aspx?id=" + dr["id"] + "'>修改</a></td>";
                str += @"        </tr>";
            }
            str += @"</table>";
            ...
            if (Request["delbox"] != null)
            {
                ...
                foreach (String _id in delidArry)
                {
                    ...
                    try
                    {
                        // B2C_ms.myDel(id);
                        // B2C_ms_M.myDel(id);
                        Response.Write("<script language='javascript'>alert('已彻底删除！');location.href='B2C_ms_List.aspx';</script>");
                    }
                    catch (Exception)
                    {
        ...
        protected void isdelBtn_ServerClick(object sender, EventArgs e)
        {
            ...
            if (Request["delbox"] != null)
            {
                ...
                foreach (String _id in delidArry)
                {
                    ...
                    {
                        // B2C_ms.setIsdel(id);
                        Response.Write("<script language='javascript'>alert('删除成功！');location.href='B2C_ms_List.aspx';</script>");
                    }
                    catch (Exception)
                    {
        ...
        protected void disnetBtn_ServerClick(object sender, EventArgs e)
        {
            ...
            if (Request["delbox"] != null)
            {
                ...
                foreach (String _id in delidArry)
                {
                    ...
                    try
                    {
                        // B2C_ms.setIsactive(id);
                        Response.Write("<script language='javascript'>alert('启用成功！');location.href='B2C_ms_List.aspx';</script>");
                    }
                    catch (Exception)
                    {
