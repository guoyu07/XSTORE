...
{
    public partial class ms_order_List : workAuth
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                try
                {
                    lt_friendly.Text = "<span class='tipsTitle'>小提示：</span> 处理您的秒杀订单";
                    //生成分页按钮
                    int _page = (Request["page"] != null ? Convert.ToInt32(Request["page"]) : 1);
                    int totalcount = Convert.ToInt32(comfun.GetDataTableBySQL("select count(*) from ms_orders").Rows[0][0]);
                    ...
                    //lb_cateadd.Text = @"<a href='B2C_ms_M_Add.aspx'>添加商品描述</a>";
                ...
                {
                    comfun.ChuliException(ex, "man/Goods/ms_order_List.cs", Session["wID"].ToString());
                }
            }
        }
        ...
        protected string ClassList(int pagesize, int page, string _sql)
        {
            ...
            sql += " ,(select M_name from b2c_mem where ms_orders.mid=b2c_mem.id) as Mname";
            sql += " ,(select st_name from b2c_sendtype where ms_orders.o_stid=b2c_sendtype.id) as stname";
            sql += ",(select a_name from B2C_order_active where id=ms_orders.aid) as aname";
            sql += " from ms_orders where ";
            ...
            str += @"<table>";
            str += @"       <tr>";
            str += "        <th ><input type=\"checkbox\" name=\"delAll\" id=\"delAll\"  onclick =\"this.value=checkAll(form1.delbox,this);\" />全选</th>";
            str += @"        <th  >时间</th>";
            str += @"        <th  >订单编号</th>";
            str += @"        <th   >金额</th>";
            str += @"        <th   >积分</th>";
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                str += @"          <td  >" + dr["o_date"] + "</td>";
                str += @"          <td  ><a href='ms_orders_Detail.aspx?ono=" + dr["o_no"].ToString() + "'>" + dr["o_no"] + "</a></td>";
        ...
        protected void delBtn_ServerClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Request["delbox"] != null)
                {
                    ...
                    string _sql = "delete from ms_orders where id in (" + delid + ")";
                    _sql += ";\r\n delete from ms_order_d where ono in (select o_no from ms_orders where id in(" + delid + "))";
                    _sql += ";\r\n delete from ms_order_addr where ono in (select o_no from ms_orders where id in(" + delid + "))";
                    _sql += ";\r\n delete from ms_order_log where ono in (select o_no from ms_orders where id in(" + delid + "))";
                    _sql += ";\r\n delete from ms_order_pay where ono in (select o_no from ms_orders where id in(" + delid + "))";

                    try
                    {
                        comfun.UpdateBySQL(_sql);
                        lt_result.Text = "已彻底删除！";
                        lt_result.Text += "<script language='javascript'>setTimeout(function(){location.href='ms_order_List.aspx';},300);</script>";
                    }
                    catch (Exception)
                    {
        ...
        protected void ss_btn_Click(object sender, EventArgs e)
        {
            ...
            //生成分页按钮
            int _page = (Request["page"] != null ? Convert.ToInt32(Request["page"]) : 1);
            int totalcount = Convert.ToInt32(comfun.GetDataTableBySQL("select count(ms_orders.id) from ms_orders where 1=1 and " + Sql).Rows[0][0]);
            lt_pagearrow.Text = Creatrue.Common.commonTool.GetArrowHtmlNew(_page, totalcount, 20, Request.Form, Request.QueryString);
