...
{
    public partial class B2C_Vip_Add : workAuth
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            lt_friendly.Text = "<span class='tipsTitle'>小提示：</span>在这里，编辑您的会员特权产品信息。";
            if (!IsPostBack)
            {
                try
                {
                    DataSet ds = comfun.GetDataSetBySQL("select g_unit from b2c_goods group by g_unit"); // where cityID=" + Session["wid"].ToString().Trim() + "
                    ...
                    if (Request["id"] != null)
                    {
                        ...
                        {
                            comfun.ChuliException(exe, "man/Goods/B2C_Vip_Add.cs", Session["wID"].ToString());
                        }
                    }
                    else
                    {
