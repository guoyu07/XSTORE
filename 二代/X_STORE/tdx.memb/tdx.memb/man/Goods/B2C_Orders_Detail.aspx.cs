...
{
    public partial class B2C_Orders_Detail : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (Session["ono"] != null)
                {
                    B2C_orders bo = new B2C_orders(_ono);
        ...
        private string GetOrderDetailList()
        {
            ...
            DataTable dat = B2C_orders.GetDetailList(_ono);
            ...
            foreach (DataRow dr in dat.Rows)
            {
                ...
                resultStr += @"          <td align=center><a href='B2C_OrderD_Mod.aspx?id=" + dr["id"] + "'>修改</a></td>";
                resultStr += @"        </tr>";
            }
            resultStr += @"</table>";
        ...
        private string GetOrderLogList()
        {
            ...
            DataTable dat = B2C_orders.GetLogsList(_ono);
        ...
        protected void btnDelete_ServerClick(object sender, EventArgs e)
        {
            ...
            if (Request["delbox"] != null)
            {
                ...
                try
                {
                    B2C_orders.deleteDetail(delid);
                    Response.Write("<script language='javascript'>alert('删除成功！');location.href='B2C_Orders_Detail.aspx?ono=" + Session["ono"].ToString().Trim() + "';</script>");
                }
                catch (Exception)
                {
        ...
        protected void btnFlag_ServerClick(object sender, EventArgs e)
        {
            if (Session["ono"] != null)
            {
                ...
                string _ono = Session["ono"].ToString().Trim();
                string _sql = "";
                _sql += B2C_orders.insertLogsSQL(_ono, "管理员", "", Convert.ToInt32(_aid));
                _sql += "update b2c_orders set aid=" + _aid + " where o_no='" + _ono + "'";

                try
                {
                    comfun.UpdateBySQL(_sql);
                    Response.Write("<script language='javascript'>alert('更新订单状态成功！');location.href='B2C_Orders_Detail.aspx?ono=" + _ono + "';</script>");
                }
                catch (Exception)
                {
        ...
        protected void btnFlag2_ServerClick(object sender, EventArgs e)
        {
            if (Session["ono"] != null)
            {
                ...
                string _sql = "";
                _sql += "update b2c_orders set o_stid=" + _stid + ",o_st_amt=" + _stAmt + ",o_allamt=o_amt+" + _stAmt + " where o_no='" + _ono + "'";

                try
                {
                    comfun.UpdateBySQL(_sql);
                    Response.Write("<script language='javascript'>alert('更新配送方式成功！');location.href='B2C_Orders_Detail.aspx?ono=" + _ono + "';</script>");
                }
                catch (Exception)
                {
        ...
        {
            Response.Redirect("B2C_Orders_Mod.aspx?ono=" + Session["ono"].ToString().Trim());
        }
