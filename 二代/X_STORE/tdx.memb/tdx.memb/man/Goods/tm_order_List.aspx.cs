...
{
    public partial class tm_order_List : workAuth
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                try
                {
                    lt_friendly.Text = "<span class='tipsTitle'>小提示：</span> 处理您的团购订单";
                    //生成分页按钮
                    int _page = (Request["page"] != null ? Convert.ToInt32(Request["page"]) : 1);
                    int totalcount = Convert.ToInt32(comfun.GetDataTableBySQL("select count(*) from tm_orders").Rows[0][0]);
                    lt_pagearrow.Text = Creatrue.Common.commonTool.GetArrowHtmlNew(_page, totalcount, 20, Request.Form, Request.QueryString);
                    lb_catelist.Text = ClassList(consts.pagesize_Txt, _page, " 1=1"); //and
                    //lb_cateadd.Text = @"<a href='B2C_team_M_Add.aspx'>添加商品描述</a>";
                ...
                {
                    comfun.ChuliException(ex, "man/Goods/tm_order_List.cs", Session["wID"].ToString());
                }
            }
        }
        ...
        protected string ClassList(int pagesize, int page, string _sql)
        {
            ...
            sql += " ,(select M_name from b2c_mem where tm_orders.mid=b2c_mem.id) as Mname";
            sql += " ,(select st_name from b2c_sendtype where tm_orders.o_stid=b2c_sendtype.id) as stname";
            sql += ",(select a_name from B2C_order_active where id=tm_orders.aid) as aname";
            sql += " from tm_orders where";
            //sql += "  cityID=" + Session["wID"].ToString().Trim();
            sql += " " + _sql;
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                str += @"          <td align=center>" + dr["o_date"] + "</td>";
                str += @"          <td align=center><a href='tm_orders_Detail.aspx?ono=" + dr["o_no"].ToString() + "'>" + dr["o_no"] + "</a></td>";
                ...
                str += "           </td>";
                str += @"          <td align=center>" + dr["stname"].ToString() + "</td>";
                str += @"          <td align=center>";
                ...
                str += "           </td>";
                str += @"          <td align=center>" + dr["aname"].ToString() + "</td>";
                str += @"        </tr>";
            }
            str += @"</table>";
        ...
        protected void delBtn_ServerClick(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Request["delbox"] != null)
                {
                    ...
                    string _sql = "delete from tm_orders where id in (" + delid + ")";
                    _sql += ";\r\n delete from tm_order_d where ono in (select o_no from tm_orders where id in(" + delid + "))";
                    _sql += ";\r\n delete from tm_order_addr where ono in (select o_no from tm_orders where id in(" + delid + "))";
                    _sql += ";\r\n delete from tm_order_log where ono in (select o_no from tm_orders where id in(" + delid + "))";
                    _sql += ";\r\n delete from tm_order_pay where ono in (select o_no from tm_orders where id in(" + delid + "))";

                    try
                    {
                        comfun.UpdateBySQL(_sql);
                        lt_result.Text = "已彻底删除！";
                        lt_result.Text += "<script language='javascript'>setTimeout(function(){location.href='tm_order_List.aspx';},300);</script>";
                    }
                    catch (Exception)
                    {
        ...
        {
            string Sql = " ";
            string key = ss_keyword.Value;
            string _cid = txt_cid.Value;

            Sql += " o_no like '%" + key + "%'"; // and
            ...
            //生成分页按钮
            int _page = (Request["page"] != null ? Convert.ToInt32(Request["page"]) : 1);
            int totalcount = Convert.ToInt32(comfun.GetDataTableBySQL("select count(tm_orders.id) from tm_orders where 1=1 and" + Sql).Rows[0][0]);
            lt_pagearrow.Text = Creatrue.Common.commonTool.GetArrowHtmlNew(_page, totalcount, 20, Request.Form, Request.QueryString);
