...
{
    public partial class B2C_OrderD_Mod : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Request["id"] != null)
                {
                    ...
                    string _sql = "select *,(select g_name from b2c_goods where id=b2c_order_d.gid) as gname from b2c_order_d where id=" + _id.ToString();
        ...
        protected void btnSave_ServerClick(object sender, EventArgs e)
        {
            ...
            if (Request["id"] != null)
            {
                try
                {
                    int _id = Convert.ToInt32(Request["id"]);
                    string _ono = "";
                    DataTable dt = comfun.GetDataTableBySQL("select ono from b2c_order_d where id=" + _id.ToString());
                    if (dt.Rows.Count > 0)
                    {
                    ...
                    dt = comfun.GetDataTableBySQL("select sum(od_amt) as amt from b2c_order_d where ono='" + _ono + "'");
                    string _odAmt = dt.Rows[0]["amt"].ToString().Trim();
                    //修改订单明细//修改订单金额
                    string _sql = "update b2c_order_d set od_price=" + _od_price.ToString() + ",od_num=" + _od_qnt.ToString() + ",od_amt=" + _od_amt.ToString() + ",od_des='" + _od_des.Replace("'", "") + "' where id=" + _id.ToString();
                    _sql += ";\r\n" + "update b2c_orders set o_amt=" + _odAmt + ",o_allamt=o_st_amt+" + _odAmt + " where o_no='" + _ono + "'";
