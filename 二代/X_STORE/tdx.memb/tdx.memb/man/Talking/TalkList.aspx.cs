...
namespace tdx.memb.man.Talking
{
    public partial class TalkList : System.Web.UI.Page
    {
        ...
        protected string ClassList(string _where, int page)
        {
            ...
            sql += "  sp.id ,sp.编号,sp.类别号,sp.名称, sp.内容,sp.创建时间,sp.是否置顶,lb.类别名   ";
            sql += "  from dbo.WP_会员表 as yh left join dbo.TK_发帖表 as sp on yh.openid=sp.openid left join TK_发帖类别表 lb on sp.类别号=lb.类别编号  ";
            sql += "    )  as aa  where aa.id  not in (select top (10*" + page + ") id  from ( select yh.openid as yhid,wx昵称 as 发布人,手机号,    ";
            sql += "  sp.id ,sp.编号,sp.类别号,sp.名称, sp.内容,sp.创建时间,sp.是否置顶,lb.类别名    ";
            sql += "  from dbo.WP_会员表 as yh left join dbo.TK_发帖表 as sp on yh.openid=sp.openid left join TK_发帖类别表 lb on sp.类别号=lb.类别编号     ";
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            goodsinfoList();
            this.ClientScript.RegisterClientScriptBlock(this.GetType(), "1", "location.href='TalkList.aspx'", true);
        }
        #endregion
