<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FerightMainEdit2.aspx.cs" Inherits="tdx.memb.man.ShoppingMall.FreightManage.FerightMainEdit2" %>
  ...
  <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    ...
    <script>
        ...
        function AjaxAdd() {
            ...
            // alert(A模板名称);
            var tableObj = document.getElementById("tabId");
            var jsonstr = "[{\"j地区\":\"默认?" + A默认id + "\",\"j首重\":\"" + A默认重量 + "\",\"j首费\":\"" + A默认元 + "\",\"j续重\":\"" + A增重 + "\",\"j续费\":\"" + A增元 + "\",\"j运送方式\":\"" + A运送方式 + "\"},";
            for (var i = 1; i < tableObj.rows.length; i++) //遍历Table的所有Row
            {
                jsonstr += "{";
                //  for (var j = 0; j < tableObj.rows[i].cells.length; j++) {   //遍历Row中的每一列
                //tableInfo += tableObj.rows[i].cells[j].innerText;   //获取Table中单元格的内容
                jsonstr += "\"j地区\":\"" + tableObj.rows[i].cells[0].getElementsByTagName("INPUT")[0].value + "?" + tableObj.rows[i].cells[0].getElementsByTagName("INPUT")[1].value + "\",\"j首重\":\"" + tableObj.rows[i].cells[1].getElementsByTagName("INPUT")[0].value + "\",\"j首费\":\"" + tableObj.rows[i].cells[2].getElementsByTagName("INPUT")[0].value + "\",\"j续重\":\"" + tableObj.rows[i].cells[3].getElementsByTagName("INPUT")[0].value + "\",\"j续费\":\"" + tableObj.rows[i].cells[4].getElementsByTagName("INPUT")[0].value + "\",\"j运送方式\":\"" + getChkbox(i, 5, 0) + "," + getChkbox(i, 5, 1) + "," + getChkbox(i, 5, 2) + "\" ";
...
<body>
 ...
 <%--           <dl>
            ...
            <dl>
                <dt>计价方式</dt>
                <dd>
                   <input type="radio" id="rdoj" runat="server" name="rdoSelect" value="0" onclick="change(this)"/>按件数
                    <input type="radio" id="rdoW" runat="server" name="rdoSelect" onclick="change(this)" value="1"/>按重量
                </dd>
            </dl>
             <dl>
                <dt>运送方式</dt>
             ...
             <div class="tdh_1_1">    <div class="tdh_2">        <asp:HiddenField ID="hidid" runat="server" />
                  默认运费:<input type="text" runat="server" id="txt默认kg" /><span id="cgTitle1">kg内</span>,<input type="text" runat="server" id="txt默认元" />元，每增加<input type="text" runat="server" id="txt增加kg" /><span id="cgTitle2">kg</span>,增加<input type="text" runat="server" id="txt增加元" />元。运送方式: <input type="checkbox" runat="server" class="ckb" value="平邮" id="ck平邮"/>平邮
                    <input type="checkbox" runat="server" class="ckb" value="快递" id="ck快递"/>快递
                    <input type="checkbox" runat="server" class="ckb" value="EMS" id="ckEMS"/>EMS </div>
                 <input type="button" id="btnAddNewRow" class="btn" value="新增" onclick="Addnewrow()"/>                
