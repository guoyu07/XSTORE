...
namespace tdx.memb.man.ShoppingMall.GoodsManage
{
    public partial class GoodsSales : System.Web.UI.Page
    {
        ...
        private void getlist(string spbh)
        {
            ...
            sql += "sp.id as 商品id,用户ID,编号,编号new,类别号,ca.c_name,品名,限购数量,规格,单位,重量,市场价,本站价,三团价,九团价,上架时间,下架时间,录入时间,库存数量,IsShow,分销率,是否上架,类型,折扣率,是否卖家承担运费,cast(满多少包邮 as decimal(18,2)) as 满多少包邮";
            sql += " from  dbo.WP_商品表 as sp   ";
            sql += " left join WP_category ca on sp.类别号=ca.c_no  left join (select b.商品id,sum(b.数量) as 销量 from [dbo].[WP_订单表] a left join [WP_订单子表] b on a.订单编号=b.订单编号 where a.state in ('未支付','已支付','已发货','已完成') " + where1 + " group by b.商品id) x on x.商品id=sp.id  ) ";
            ...
            {
                Rp_UserInfo.DataSource = dtdata.DefaultView;
        ...
        protected string ClassList(string _where, int page)
        {
            ...
            sql += "  sp.id as 商品id,用户ID,编号,编号new,类别号,ca.c_name,品名,规格,限购数量,单位,重量,分销率,市场价,本站价,三团价,九团价,上架时间,下架时间,录入时间,库存数量,IsShow,是否上架,类型,折扣率,是否卖家承担运费,cast(满多少包邮 as decimal(18,2)) as 满多少包邮   ";
            sql += "   from  dbo.WP_商品表 as sp    ";
            sql += "    left join WP_category ca on sp.类别号=ca.c_no  left join (select b.商品id,sum(b.数量) as 销量 from [dbo].[WP_订单表] a left join [WP_订单子表] b on a.订单编号=b.订单编号 where   a.state in ('未支付','已支付','已发货','已完成') " + where1 + " group by b.商品id) x on x.商品id=sp.id  ) as aa  where aa.IsShow=1 and aa.商品id not in (select top (10*" + page + ") aa.商品id  from ( select isnull(x.销量,0) as 销量,    ";
            sql += "  sp.id as 商品id,用户ID,编号,编号new,类别号,ca.c_name,品名,规格,单位,重量,分销率,市场价,本站价,三团价,九团价,上架时间,下架时间,录入时间,库存数量,IsShow,是否上架    ";
            sql += "   from  dbo.WP_商品表 as sp    ";
            sql += "    left join WP_category ca on sp.类别号=ca.c_no  left join (select b.商品id,sum(b.数量) as 销量 from [dbo].[WP_订单表] a left join [WP_订单子表] b on a.订单编号=b.订单编号 where  a.state in ('未支付','已支付','已发货','已完成') " + where1 + " group by b.商品id) x on x.商品id=sp.id )  as aa  where  aa.IsShow=1 and  " + _where + "";
            sql += "  order by 销量 desc) and  " + _where + " ";
            sql += "  order by 销量 desc  ";



            DataTable dtdata = DbHelperSQL.Query(sql).Tables[0];
        ...
        private void goods(string where)
        {
            ...
            sql += "sp.id as 商品id,用户ID,编号,编号new,类别号,ca.c_name,品名,规格,单位,限购数量,重量,分销率,市场价,本站价,三团价,九团价,上架时间,下架时间,录入时间,库存数量,IsShow,是否上架,类型,折扣率,是否卖家承担运费,cast(满多少包邮 as decimal(18,2)) as 满多少包邮";
            sql += " from  WP_商品表 as sp    ";
            sql +=
                " left join WP_category ca on sp.类别号=ca.c_no  left join (select b.商品id,sum(b.数量) as 销量 from [dbo].[WP_订单表] a left join [WP_订单子表] b on a.订单编号=b.订单编号 where a.state in ('未支付','已支付','已发货','已完成') " +
                where1 + " ";
            ...
            try{
                ...
                int totalcount = Convert.ToInt32(DbHelperSQL.Query(sql).Tables[0].Rows[0][0]);
        ...
        private DataTable sousuo(string where)
        {
            string sql = "select *  from ( select isnull(x.销量,0) as 销量,  ";
            sql += "sp.id as 商品id,用户ID,编号,编号new,类别号,ca.c_name,品名,规格,限购数量,单位,重量,分销率,市场价,本站价,三团价,九团价,上架时间,下架时间,录入时间,库存数量,IsShow,是否上架,类型,折扣率,是否卖家承担运费,cast(满多少包邮 as decimal(18,2)) as 满多少包邮";  
            sql += " from  dbo.WP_商品表 as sp  ";
            sql += " left join WP_category ca on sp.类别号=ca.c_no  left join (select b.商品id,sum(b.数量) as 销量 from [dbo].[WP_订单表] a left join [WP_订单子表] b on a.订单编号=b.订单编号 where  a.state in ('未支付','已支付','已发货','已完成') " + where1 + " group by b.商品id) x on x.商品id=sp.id )  as aa  where aa.IsShow=1 and 1=1  " + where + " order by 销量 desc ";
        ...
        protected void LBtn_Export_Click(object sender, EventArgs e)
        {
            ...
            string sql = "select 编号new as 商品编号,品名,c_name as 类别,单位,规格,重量,市场价,本站价,库存数量,限购数量,上架时间,下架时间,分销率,折扣率,销量  from ( select isnull(x.销量,0) as 销量, ";
            sql += " sp.id as 商品id,用户ID,编号,编号new,类别号,ca.c_name,品名,规格,限购数量,单位,重量,分销率,市场价,本站价,三团价,九团价,上架时间,下架时间,录入时间,库存数量,IsShow,是否上架,类型,折扣率,是否卖家承担运费,cast(满多少包邮 as decimal(18,2)) as 满多少包邮";       
            sql += " from  dbo.WP_商品表 as sp  ";
            sql += " left join WP_category ca on sp.类别号=ca.c_no  left join (select b.商品id,sum(b.数量) as 销量 from [dbo].[WP_订单表] a left join [WP_订单子表] b on a.订单编号=b.订单编号 where  a.state in ('未支付','已支付','已发货','已完成') " + where2 + " group by b.商品id) x on x.商品id=sp.id )  as aa  where aa.IsShow=1 and 1=1  order by 销量 desc ";
