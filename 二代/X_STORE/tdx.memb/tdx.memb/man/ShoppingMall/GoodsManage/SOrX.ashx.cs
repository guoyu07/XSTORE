...
namespace tdx.memb.man.ShoppingMall.GoodsManage
{
    ...
    /// SOrX 的摘要说明
    /// </summary>
    public class SOrX : IHttpHandler
    {
        ...
        public int YesNO(int id)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                if (dt.Rows[0]["是否卖家承担运费"].ToString() == "0")
                {
                    //string sql = "update dbo.WP_商品表 set 是否上架='1' where id=" + id;
                    string sql = "update dbo.WP_商品表 set 是否卖家承担运费='1' where 编号 in (select 编号 from WP_商品表 where id=" + id + ")";
                ...
                else
                {
                    ...
                    string sql = "update dbo.WP_商品表 set 是否卖家承担运费='0' where 编号 in (select 编号 from WP_商品表 where id=" + id + ")";
