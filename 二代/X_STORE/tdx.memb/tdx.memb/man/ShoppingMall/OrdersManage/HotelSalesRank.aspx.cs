...
{
    public partial class HotelSalesRank : System.Web.UI.Page
    {
        ...
        protected void PageInit(string sort = "")
        {
            ...
            {
                whereSql += string.Format("and 酒店名称 like '%{0}%'", keyWord);
            }
            if (beginAmount != 0)
            {
            ...
            var saleRankSql =
                string.Format(
                    @"SELECT   酒店名称, 房间数, ISNULL(销量, 0) AS 销量, ISNULL(金额, 0) AS 金额, ISNULL(日均房, 0) AS 日均房, 首次入驻
FROM      (SELECT   仓库名 AS 酒店名称,
                                     (SELECT   COUNT(*) AS Expr1
                                      FROM      dbo.WP_库位表
                                      WHERE   (仓库id = a.id) AND (状态 IN (1, 2)) AND (箱子MAC IS NOT NULL) AND (箱子MAC <> '') AND 库位名 NOT LIKE '%总台%') AS 房间数,
                                     (SELECT   SUM(dbo.WP_订单子表.数量) AS Expr1
                                      FROM      dbo.WP_订单子表 LEFT OUTER JOIN
                                                      dbo.WP_订单表 ON dbo.WP_订单子表.订单编号 = dbo.WP_订单表.订单编号
                                      WHERE   (dbo.WP_订单子表.仓库id = a.id) AND (dbo.WP_订单表.state IN (3, 5)) AND {0}) AS 销量,
                                     (SELECT   SUM(WP_订单子表_2.价格) AS Expr1
                                      ...
                                      WHERE   (WP_订单子表_2.仓库id = a.id) AND (WP_订单表_2.state IN (3, 5)) AND {3}) AS 金额, dbo.DayAverage(id,'{1}','{2}') 
                                 AS 日均房,
                                     (SELECT   TOP (1) WP_订单子表_1.下单时间
                                      ...
                                      WHERE   (WP_订单子表_1.仓库id = a.id) AND (WP_订单表_1.state IN (3, 5))
                                      ORDER BY WP_订单子表_1.下单时间) AS 首次入驻
                 FROM  dbo.WP_仓库表 AS a where IsShow = 1 and (SELECT   COUNT(*) 
                                      FROM      dbo.WP_库位表
                                      WHERE   (仓库id = a.id) AND (状态 IN (1, 2)) AND (箱子MAC IS NOT NULL) AND (箱子MAC <> '') AND 库位名 NOT LIKE '%总台%')!=0 ) AS b", saleRankWhereSql,beginTime,endTime, amountRankWhereSql);
            ...
            var selectSql = string.Format(@"select 酒店名称,房间数,销量,金额,日均房,首次入驻 from ({1}) as c  where {0}", whereSql,saleRankSql);
            ...
            var _sql = string.Format(@"select isnull(sum(销量),1) as 总销量,sum(金额) as 总金额,sum(房间数) as 总房间数,case sum(销量) when 0 then 0 else (sum(金额)/sum(销量)) end as 客单价 from ({1}) as c where {0}", whereSql,saleRankSql);
        ...
        protected void btnExport_OnClick(object sender, EventArgs e)
        {
            ...
            DTcms.Common.NPOIHelper.RenderDataTableToExcel_Web(dt, "酒店销售排行" + DateTime.Now.ToString("yyyy-MM-dd"));
        }
        #endregion
    }
}