...
namespace tdx.memb.man.ShoppingMall.OrdersManage
{
    public partial class OrdersBigList : System.Web.UI.Page
    {
        ...
        protected string ClassList(string _where, int page)
        {
            ...
            sql += " from  dbo.WP_订单表  as wd  left join    ";
            sql += "   dbo.WP_订单支付表 as dd on wd.订单编号=dd.订单编号 left join    ";
        ...
        private void goods(string where)
        {
            ...
            string sql = "select count(*) from wp_订单表";
            #region 分页2015.6.25
            try
            {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < Rp_UserInfo.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    int count1 = comfun.DelbySQL("delete from wp_订单表 where id=" + id);
                    int count2 = comfun.DelbySQL("delete from wp_订单子表 where 订单编号='" + no + "'");
        ...
        protected void lbtchange_state_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < Rp_UserInfo.Items.Count; i++)
                {
                    ...
                    {
                        flag = true;
                        comfun.UpdateBySQL("update wp_订单表 set state='" + state + "' where id=" + id + " and 订单编号='" + no + "'");
                    }
