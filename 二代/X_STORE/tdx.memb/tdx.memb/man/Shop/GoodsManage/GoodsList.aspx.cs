...
namespace tdx.memb.man.Shop.GoodsManage
{
    public partial class GoodsList : System.Web.UI.Page
    {
        ...
        private void getstore()
        {
            ...
            dt1.Rows.Add("全部", "全部");
            dt1.Rows.Add("新世界", "新世界");
            dt1.Rows.Add("金桥", "金桥");
            ddl_store.DataSource = dt1;
            ddl_store.DataTextField = "name";
        ...
        {
            string sql = "select  *  from ( select  isnull((select  salenum from  [SaleCount] where   SaleCount.商品id=sp.id and  typename='WP'),0) as salenum, ";
            sql += "sp.id as 商品id,用户ID,编号,编号new,类别号,ca.c_name,品名,规格,单位,重量,市场价,本站价,序号,分销率,三团价,九团价,上架时间,下架时间,录入时间,库存数量,限购数量,IsShow,是否上架,类型,折扣率,是否卖家承担运费,cast(满多少包邮 as decimal(18,2)) as 满多少包邮,";
            sql += "sp.regtime,(select top 1 图片路径 from WP_商品图片表 where sp.编号=商品编号 order by 序号 asc) as 图片路径 ";
        ...
        protected string ClassList(string _where, int page)
        {
            ...
            string sql = "  select top 8 *  from ( select  isnull((select  salenum from  [SaleCount] where   SaleCount.商品id=sp.id and  typename='WP'),0) as salenum,   ";
            sql += "  sp.id as 商品id,用户ID,编号,编号new,类别号,ca.c_name,品名,规格,单位,重量,市场价,本站价,序号,分销率,三团价,九团价,上架时间,下架时间,录入时间,库存数量,限购数量,IsShow,是否上架,类型,折扣率,是否卖家承担运费,cast(满多少包邮 as decimal(18,2)) as 满多少包邮,    ";
            sql += "  sp.regtime,(select top 1 图片路径 from WP_商品图片表 where sp.编号=商品编号 order by 序号 asc) as 图片路径    ";
            sql += "   from  dbo.WP_商品表 as sp      ";
            ...
            sql += "  sp.id as 商品id    from  dbo.WP_商品表 as sp   ";
            sql += "     left join WP_category ca on sp.类别号=ca.c_no   where  IsShow=1 and  " + _where + " ";
            sql += " " + where2 + " order by 序号 asc, sp.regtime desc) and aa.IsShow=1  and  " + _where + " ";
        ...
        private void goods(string where)
        {
            ...
            //sql += "on sp.编号=spxq.商品编号 left join dbo.WP_商品图片表  as sptp on  spxq.商品编号=sptp.商品编号)  as aa  "+where;

            string sql = "select count(*) from ( select   ";
            sql += "\r\n sp.id as 商品id,用户ID,编号,编号new,类别号,ca.c_name,品名,规格,单位,重量,市场价,本站价,分销率,三团价,九团价,上架时间,下架时间";
            sql += "\r\n ,录入时间,库存数量,限购数量,IsShow,是否上架,类型,折扣率,是否卖家承担运费,cast(满多少包邮 as decimal(18,2)) as 满多少包邮,";
            sql += "\r\n sp.regtime,(select top 1 图片路径 from WP_商品图片表 where sp.编号=商品编号 order by 序号 asc) as 图片路径  ";
            //sql += "from dbo.dt_manager as yh left join dbo.WP_商品表 as sp on yh.id=sp.用户id   left join dbo.WP_商品详情表  as spxq  ";
            sql += "\r\n from  dbo.WP_商品表 as sp     ";
        ...
        protected void LBtn_sousuo_Click(object sender, EventArgs e)
        {
            ...
            if (ddl_store.SelectedValue != "全部")
            {
                query += " and store_name like '%" + ddl_store.SelectedValue.Trim() + "%'   ";
                //sousuo( "品名 like '%"+ txt_pinming.Text.Trim()+"%'");
            }
        ...
        {
            string sql = "select *  from ( select  isnull((select  salenum from  [SaleCount] where   SaleCount.商品id=sp.id and  typename='WP' ),0) as salenum, ";
            sql += "sp.id as 商品id,用户ID,编号,编号new,类别号,ca.c_name,品名,规格,单位,重量,市场价,本站价,序号,分销率,三团价,九团价,上架时间,下架时间,录入时间,库存数量,限购数量,IsShow,是否上架,类型,折扣率,是否卖家承担运费,cast(满多少包邮 as decimal(18,2)) as 满多少包邮,store_name,";
            sql += "sp.regtime,(select top 1 图片路径 from WP_商品图片表 where sp.编号=商品编号 order by 序号 asc) as 图片路径 ";
            sql += "from   dbo.WP_商品表 as sp      ";
            sql += " left join WP_category ca on sp.类别号=ca.c_no   )  as aa  where aa.IsShow=1 and 1=1   " + where + "  ";
            sql += "" + where1 + "  order by 序号 asc, regtime desc";
        ...
        {
            string sql = "select  	c_name as 类别,品名,规格,编号new as 条形码,单位,重量,市场价,本站价,序号 as 显示顺序,分销率,上架时间,下架时间,库存数量,限购数量,case 是否上架 when 1 then '是' else '否' end as 是否上架,折扣率,case 是否卖家承担运费 when 1 then '是' else '否' end as 是否卖家承担运费,regtime as 更新时间 from ( select   isnull((select  salenum from  [SaleCount] where   SaleCount.商品id=sp.id and  typename='WP'),0) as salenum, ";
            sql += "sp.id as 商品id,用户ID,编号,编号new,类别号,ca.c_name,品名,规格,单位,重量,市场价,本站价,序号,分销率,三团价,九团价,上架时间,下架时间,录入时间,库存数量,限购数量,IsShow,是否上架,类型,折扣率,是否卖家承担运费,cast(满多少包邮 as decimal(18,2)) as 满多少包邮,";
            sql += "sp.regtime  ";
            sql += "from  dbo.WP_商品表 as sp  ";
            sql += " left join WP_category ca on sp.类别号=ca.c_no   )  as aa  where aa.IsShow=1 and 1=1    " + where1 + " order by 序号 asc,regtime desc ";

            DataTable dt1 = comfun.GetDataTableBySQL(sql);

            if (dt1.Rows.Count > 0)
            {
                DTcms.Common.Excel.DataTable4Excel(dt1, "商品表");
            }
            else
