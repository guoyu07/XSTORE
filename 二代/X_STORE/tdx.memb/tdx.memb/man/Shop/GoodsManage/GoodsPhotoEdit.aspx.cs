...
namespace tdx.memb.man.Shop.GoodsManage
{
    public partial class GoodsPhotoEdit : System.Web.UI.Page
    {
        ...
        DTcms.Model.WP_商品图片表 sptpmodel = new DTcms.Model.WP_商品图片表();
                ...
                if (dtid > 0)
                {
                    ...
                    if (dtdt.Rows.Count > 0)
                    {
                        for (int i = 0; i < dtdt.Rows.Count; i++)
                        {
                            ...
                            //{
                                //string sql = " select * from dbo.WP_商品表 where 用户ID=" + dtdt.Rows[0]["id"];
                            string sql = " select * from dbo.WP_商品表 where 用户ID !=0";
