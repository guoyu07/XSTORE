...
namespace tdx.memb.man.Shop.OrdersManage
{
    public partial class OrderListNew : System.Web.UI.Page
    {
        ...
        private void loadinfo(string opid)
        {
            ...
            sbsql_total.Append("\r\n   select * from                                                                          ");
            sbsql_total.Append("\r\n     (select count(1) as 全部  from  [dbo].[WP_订单表] where 1=1  " + wherenew + " ) as 全部                         ");
            sbsql_total.Append("\r\n   ,(select count(1) as 未支付 from  [dbo].[WP_订单表] where [state]='未支付' "+wherenew+") as 未支付  ");
            sbsql_total.Append("\r\n   ,(select count(1) as 待发货 from  [dbo].[WP_订单表] where [state]='已支付' "+wherenew+") as 待发货  ");
            sbsql_total.Append("\r\n   ,(select count(1) as 已发货 from  [dbo].[WP_订单表] where [state]='已发货' "+wherenew+") as 已发货  ");
            sbsql_total.Append("\r\n   ,(select count(1) as 已完成 from  [dbo].[WP_订单表] where [state]='已完成' "+wherenew+") as 已完成  ");
            sbsql_total.Append("\r\n   ,(select count(1) as 已取消 from  [dbo].[WP_订单表] where [state]='已取消' "+wherenew+") as 已取消  ");

            sbsql_total.Append("\r\n   ,(select count(1) as 退款中 from  [dbo].[WP_订单表] where [state]='退款中' "+wherenew+") as 退款中  ");
            sbsql_total.Append("\r\n   ,(select count(1) as 退款成功 from  [dbo].[WP_订单表] where [state]='退款成功' "+wherenew+") as 退款成功  ");
            ...
            sborder.Append("\r\n ,(select 收货人 from [dbo].[WP_订单地址表] where id in (select 地址id from WP_地址表 where WP_地址表.订单编号=WP_订单表.订单编号) ) as 收货人 from   WP_订单表 " + (string.IsNullOrEmpty(orderstate) ? "" : "where state='" + orderstate + "' ") + ") t1  left join  ");
            ...
            { 
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    sbsql.Append("\r\n     select * from  (select  订单编号,价格,数量 ,(select top 1 图片路径 from [dbo].[WP_商品图片表]  ");
                    sbsql.Append("\r\n where 商品编号=(select 编号  from WP_商品表 where WP_商品表.id=WP_订单子表.商品id) order by 序号 desc) as  商品图片");
                    sbsql.Append("\r\n ,(select top 1 编号new from  WP_商品表 where WP_商品表.id=WP_订单子表.商品id)+'-'+(select top 1 品名 from  WP_商品表 where WP_商品表.id=WP_订单子表.商品id)+'('+(select top 1 规格 from  WP_商品表 where WP_商品表.id=WP_订单子表.商品id)+')' as  品名  ");
                    sbsql.Append("\r\n     ,(select  convert(varchar(50),下单时间,23)  as 下单时间  from  WP_订单表 where WP_订单表.订单编号=WP_订单子表.订单编号)  as 下单时间    ");
                    sbsql.Append("\r\n     ,(select [state] from    [dbo].[WP_订单表] where   WP_订单表.订单编号  = WP_订单子表.订单编号 ) as 订单状态   ");
                    sbsql.Append("\r\n     from [dbo].[WP_订单子表] ) t    ");
                    sbsql.Append("\r\n     where 订单编号='" + dt.Rows[i]["订单编号"] + "' ");
                    ...
                    for (int j = 0; j < dorder.Rows.Count; j++)
                    {
                        if (j == 0)
                        {
                            ...
                            {
                                //sbhtml.Append("\r\n    <a href=\"../../PrintSheet/ShowPrintInfo.aspx?orderNO=" + dt.Rows[i]["订单编号"].ToString() + "&orderType=WP\" >打印订单</a><br/>");
                            ...
                            if (dt.Rows[i]["状态"].ToString() == "已发货" || dt.Rows[i]["状态"].ToString() == "已完成")
                            {
                                ...
                                sbhtml.Append("\r\n    <input type=button id=look  value='查看物流信息' onclick=\"look_wuliu('" + dt.Rows[i]["物流公司"].ToString() + "','" + dt.Rows[i]["物流单号"].ToString() + "')\"/>");
                                //sbhtml.Append("\r\n    <input type=button id=wuliu  value='修改物流信息' type='hidden' onclick=\"popupDiv('pop-div1',this);\" gs=\"" + gs + "\" dh=\"" + dh + "\"/>");
                                Text3.Value = Utils.ObjectToStr(dt.Rows[i]["物流公司"]);//物流公司
                                Text4.Value = Utils.ObjectToStr(dt.Rows[i]["物流单号"]);//物流单号
                            }
        ...
        protected void lbtn_Export_Click(object sender, EventArgs e)
        {
            ...
            sbsql.Append("\r\n     (select top 1 品名 from  WP_商品表 where WP_商品表.id=WP_订单子表.商品id)  as  商品名称   ");
            sbsql.Append("\r\n     ,(select top 1 编号new from  WP_商品表 where WP_商品表.id=WP_订单子表.商品id)  as  条形码  ");
            sbsql.Append("\r\n     ,(select top 1 规格 from  WP_商品表 where WP_商品表.id=WP_订单子表.商品id)  as  商品规格 ");
            sbsql.Append("\r\n     ,(select [state] from    [dbo].[WP_订单表] where   WP_订单表.订单编号  = WP_订单子表.订单编号 ) as 订单状态    ");
            sbsql.Append("\r\n     from [dbo].[WP_订单子表])   t ");
        ...
        protected void lbtn_ExpsortSum_Click(object sender, EventArgs e)
        {
            ...
            sbsql.Append("\r\n       	(select 数量,                                                                                                                                  ");
            sbsql.Append("\r\n       		(select top 1 品名 from  WP_商品表 where WP_商品表.id=WP_订单子表.商品id) as  商品名称 ,订单编号                                      ");
            sbsql.Append("\r\n       		,(select top 1 编号new from  WP_商品表 where WP_商品表.id=WP_订单子表.商品id) as  条形码                                                   ");
            sbsql.Append("\r\n       		,(select top 1 规格 from  WP_商品表 where WP_商品表.id=WP_订单子表.商品id) as  规格                                                   ");
            sbsql.Append("\r\n       		,价格 as 单价,(select  运费  from WP_订单表 where WP_订单表.订单编号=WP_订单子表.订单编号) as 运费                                    ");
            sbsql.Append("\r\n       		,(select [state] from    [dbo].[WP_订单表] where   WP_订单表.订单编号  = WP_订单子表.订单编号 ) as 订单状态                           ");
            sbsql.Append("\r\n                ,(select 下单时间 from    [dbo].[WP_订单表] where   WP_订单表.订单编号  = WP_订单子表.订单编号 ) as 下单时间                        ");
            sbsql.Append("\r\n       		,(select  总金额  from WP_订单表 where WP_订单表.订单编号=WP_订单子表.订单编号) as 订单金额  ");
            sbsql.Append("\r\n       					,(select  应付款  from WP_订单表 where WP_订单表.订单编号=WP_订单子表.订单编号) as 订单总金额 ");
            sbsql.Append("\r\n      		,( select 优惠金额  from  (select isnull(sum(q_money),0) as 优惠金额,orderNo as 订单编号 from TM_quan_mem_log a left join TM_Quan_mem b on a.qmid=b.id left join TM_Quan c on b.qid=c.id group by orderNo) t where t.订单编号=WP_订单子表.订单编号) as 优惠金额          ");
            sbsql.Append("\r\n       		from [dbo].[WP_订单子表]                                                                                                              ");
            sbsql.Append("\r\n       	) t1    " + wherenew + "                                                                                                               ");
            ...
            sbsql.Append("\r\n      ) t3                                                                                                                                         ");
            sbsql.Append("\r\n      left join                                                                                                                                    ");
            sbsql.Append("\r\n      (select  WP_订单表.订单编号,t4.手机号,t4.收货人,t4.收货地址,下单时间,备注,'商品链接' as 商品链接,                                            ");
            sbsql.Append("\r\n      (select top 1 M_name from B2C_mem where openid=WP_订单表.openid) as 下单人                                                                   ");
            sbsql.Append("\r\n      from  [dbo].[WP_订单表]   left join                                                                                                          ");
            sbsql.Append("\r\n      (select   WP_地址表.订单编号, 手机号,收货人, 省+市+区+isnull(商圈,'')+详细地址 as 收货地址                                                   ");
            sbsql.Append("\r\n      from [dbo].[WP_订单地址表]  left join [dbo].[WP_地址表] on WP_订单地址表.id=WP_地址表.地址id                                                     ");
            sbsql.Append("\r\n      ) t4 on  WP_订单表.订单编号=t4.订单编号) t5                                                                                                  ");
            sbsql.Append("\r\n      on t5.订单编号=t3.订单编号                                                                                                                   ");
            sbsql.Append("\r\n      left join                                                                                                                                    ");
            sbsql.Append("\r\n      (                                                                                                                                            ");
            sbsql.Append("\r\n      select  订单编号,支付方式,支付金额 ,convert( varchar(50),支付时间,21) as 支付时间                                                            ");
            sbsql.Append("\r\n      from  WP_订单支付表          where  支付方式='微信' or 支付方式='支付宝'                                                                  ");
