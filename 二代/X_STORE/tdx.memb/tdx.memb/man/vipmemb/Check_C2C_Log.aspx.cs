...
{
    public partial class Check_C2C_Log : workAuth
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (Request["id"] != null)
                {
                    ...
                    string _where = string.Format(" wid={0} and uid={1} order by id desc", _wid, _id);
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        else
                        {
                            result1 += " \r\n <td>" + "钱包" + "</td> ";
                            if (Convert.ToDouble(dr["ac_money"].ToString()) >= 0)
                                result1 += " \r\n  <td>" + "钱包充值:" + Convert.ToDouble(dr["ac_money"].ToString()).ToString("F2") + "</td> ";
                            else
                                result1 += " \r\n  <td>" + "钱包消费:" + ((-1) * Convert.ToDouble(dr["ac_money"].ToString())).ToString("F2") + "</td> ";
                        }
