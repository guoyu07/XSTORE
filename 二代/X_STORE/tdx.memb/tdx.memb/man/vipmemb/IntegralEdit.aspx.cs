...
{
    public partial class IntegralEdit : workAuth
    {
            ...
            if (verify())
            {
                if (Request["id"] != null && Session["wid"] != null)
                {
                    try
                    {
                        ...
                        _bw.Update();
                        commonTool.Show_Have_Url(lt_result, "修改成功！", "Integral.aspx", 0);
                    }
                    catch (Exception ex)
                    {
                ...
                else if (Session["wid"] != null)
                {
                    ...
                    try
                    {
                        ...
                        _bw.Update();
                        commonTool.Show_Have_Url(lt_result, "添加成功！", "Integral.aspx", 0);
                    }
                    catch
                    {
        ...
        internal bool verify()
        {
            ...
            if (!double.TryParse(this.amount.Value.ToString(), out amount))
            {
                isTrue = false;
                commonTool.Show_Have_Url(lt_result, "产生的费用输入不规范！", "", 1);

            }
            else if (!double.TryParse(this.give_amount.Value.ToString(), out give_amount))
            {
                isTrue = false;
                commonTool.Show_Have_Url(lt_result, "赠送的积分输入不规范！", "", 1);
            }
            else if (amount < 0)
            {
                isTrue = false;
            ...
            {
                isTrue = false;
                commonTool.Show_Have_Url(lt_result, "赠送的积分不能为负数！", "", 1);
            }
