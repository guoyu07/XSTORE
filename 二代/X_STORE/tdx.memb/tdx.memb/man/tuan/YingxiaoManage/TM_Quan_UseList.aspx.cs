...
{
    public partial class TM_Quan_UseList : DTcms.Web.UI.ManagePage
    {
        private int qid = -1;
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                string sql = " qmid in (select id from  [TM_Quan_mem] where qid="+qid+") ";
                ...
                int totalcount = Convert.ToInt32(comfun.GetDataTableBySQL("select count(*) from  TM_quan_mem_log where " + sql).Rows[0][0]);
        ...
        private string prolist(string _dzd, string _tname, string _sql, int _page)
        {
            ...
            DataTable dt = database.TM_Quan_mem_log.GetList(currentpage, _dzd, _tname, _sql);
