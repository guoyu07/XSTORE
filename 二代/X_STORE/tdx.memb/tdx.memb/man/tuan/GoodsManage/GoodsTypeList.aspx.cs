...
namespace tdx.memb.man.Tuan.GoodsManage
{
    ...
    {
        DTcms.BLL.TM_商品类别表 splbbll = new DTcms.BLL.TM_商品类别表();
       ...
       public void  goodstypeshow()
       {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < Rp_UserInfo.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    DTcms.Model.TM_商品类别表 model = splbbll.GetModel(id);
                    bool res = splbbll.Delete(id);
                    string typeno = model.类别编号;
                    List<DTcms.Model.TM_商品表> goods = new DTcms.BLL.TM_商品表().GetModelList(" 类别号='" + typeno + "'");
                    if (goods != null)
                    {
                        foreach (DTcms.Model.TM_商品表 g in goods)
                        {
                            new DTcms.BLL.TM_商品表().Delete(g.id);
                            List<DTcms.Model.TM_商品图片表> imgs = new DTcms.BLL.TM_商品图片表().GetModelList(" 商品编号='" + g.编号+"'");
                            if (imgs != null)
                            {
                                foreach (DTcms.Model.TM_商品图片表 m in imgs)
                                {
                                    new DTcms.BLL.TM_商品图片表().Delete(m.id);
                                }
                            }
                        }
                    }
