...
namespace tdx.memb.man.Tuan.GoodsManage
{
    ...
    {
        DTcms.BLL.TM_商品图片表 spbll = new DTcms.BLL.TM_商品图片表();
        ...
        protected string ClassList(string _where, int page)
        {
            ...
            string sql = "  select top 10*  from  dbo.TM_商品图片表 where id not in (select top (10*" + page + ") id  from  dbo.TM_商品图片表  where 商品编号 in(select 编号 from dbo.TM_商品表 " + _where + " ) order by id desc) and     ";
            sql += "  商品编号 in(select 编号 from dbo.TM_商品表  " + _where + ") order by id desc   ";
        ...
        {
            string sql = " select * from dbo.TM_商品表 " + p;
        ...
        private void goods(string where)
        {
            ...
            string sql = " select * from  dbo.TM_商品图片表 where 商品编号 in(select 编号 from dbo.TM_商品表 " + where + ") order by id desc";
                ...
                if (dtdt.Rows.Count > 0)
                {
                    for (int i = 0; i < dtdt.Rows.Count; i++)
                    {
                        if (dtdt.Rows[i]["user_name"].ToString() != "admin")
                        {
                            ...
                            //try
                            //{
                            //    string sql = " select * from  dbo.TM_商品图片表 where 商品编号 in(select 编号 from dbo.TM_商品表  where 编号=" + drp_photo.SelectedValue + ") order by id desc";
                            //    int _page = (Request["page"] != null ? Convert.ToInt32(Request["page"]) : 1);
        ...
        {
            string sql = " select * from  dbo.TM_商品图片表 where 商品编号 in(select 编号 from dbo.TM_商品表 " + where + ") " + s + " order by id desc";
