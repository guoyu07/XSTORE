...
namespace tdx.memb.man.Tuan.OrdersManage
{
    public partial class ZiTi : System.Web.UI.Page
    {
        ...
        protected string ClassList(string _where, int page)
        {
            ...
            string sql = " select top 10* from TM_订单表 a,TM_订单子表 zb,TM_地址表 b ,TM_订单支付表 c ";
            sql += "where a.openid in (select openid from TM_地址表) ";
            sql += "and a.订单编号 = b.订单编号 and a.订单编号 =c.订单编号 ";
            sql += "and 地址ID=0 and zb.商品编号 in(select 编号 from dbo.TM_商品表  " + _where + "    ) ";
            sql += "and a.id not in (select top (10*" + page + ")  a.id  from TM_订单表 a,TM_订单子表 zb,TM_地址表 b ,TM_订单支付表 c ";
            sql += " where a.openid in (select openid from TM_地址表) ";
            sql += " and a.订单编号 = b.订单编号 and a.订单编号 =c.订单编号  and 地址ID=0 ";
            sql += " and zb.商品编号 in(select 编号 from dbo.TM_商品表  " + _where + "    ) order by a.id desc)  order by a.id desc";
        ...
        private void goods(string where)
        {
            ...
            string sql = "select  count(*)  from TM_订单表 a,TM_订单子表 zb,TM_地址表 b ,TM_订单支付表 c";
            sql += " where a.openid in (select openid from TM_地址表)";
            sql += " and a.订单编号 = b.订单编号";
            sql += " and a.订单编号 =c.订单编号 ";
            sql += " and 地址ID=0 and zb.商品编号 in(select 编号 from dbo.TM_商品表 " + where + "  ) ";
        ...
        //                if (dtdt.Rows[i]["user_name"].ToString() != "admin")
        //                {
        //                    string sql = "select * from TM_订单表 a,TM_地址表 b ,TM_订单支付表 c";
        //                    sql += " where a.openid in (select openid from TM_地址表)";
        //                    sql += " and a.订单编号 = b.订单编号";
        //                    sql += " and a.订单编号 =c.订单编号 ";
        //                    sql += " and 地址ID=0 and 商品编号 in(select 编号 from dbo.TM_商品表  where 用户ID=" + dtdt.Rows[i]["id"].ToString() + ")  ";
        ...
        //                    //sql += "from dbo.TM_订单表 as d left join ";
        //                    //sql += "dbo.TM_订单地址表 as dd on d.订单编号=dd.订单编号 left join ";
        //                    //sql += "dbo.TM_订单支付表 as dz   on dd.订单编号=dz.订单编号 ";
        //                    //sql += "where 商品编号 in(select 编号 from dbo.TM_商品表  where 用户ID=" + dtdt.Rows[i]["id"].ToString() + ")) as aa ";
        ...
        //                    //sql += "dbo.TM_订单地址表 as dd on d.订单编号=dd.订单编号 left join ";
        //                    //sql += "dbo.TM_订单支付表 as dz   on dd.订单编号=dz.订单编号 ";
        //                    //sql += "where 商品编号 in(select 编号 from dbo.TM_商品表 )) as aa ";

        //                    string sql = "select * from TM_订单表 a,TM_地址表 b ,TM_订单支付表 c";
        //                    sql += " where a.openid in (select openid from TM_地址表)";
        //                    sql += " and a.订单编号 = b.订单编号";
        //                    sql += " and a.订单编号 =c.订单编号 ";
        //                    sql += " and 地址ID=0 and 商品编号 in(select 编号 from dbo.TM_商品表  ) ";
        //                    DataTable dt = DbHelperSQL.Query(sql).Tables[0];
        //                    if (dt.Rows.Count > 0)
        //                    {
        //                        DTcms.Common.Excel.DataTable4Excel(dt, "订单Excel表");
        ...
        public void sousuo(string where)
        {
            ...
            sql += " and a.订单编号 = b.订单编号";
            sql += " and a.订单编号 =c.订单编号 ";
            sql += " and 地址ID=0 and 商品编号 in(select 编号 from dbo.TM_商品表  ) " + where;
            ...
            //sql += "dbo.TM_订单地址表 as dd on d.订单编号=dd.订单编号 left join ";
            //sql += "dbo.TM_订单支付表 as dz   on dd.订单编号=dz.订单编号 ";
            //sql += "where 商品编号 in(select 编号 from dbo.TM_商品表 )) as aa " + where;
        ...
        {
            string sql = "select * from dbo.TM_商品表 where 编号='" + bianhao + "'";
        ...
        {
            string sql = "select * from  dbo.TM_会员表 where openid='" + openid + "' ";
