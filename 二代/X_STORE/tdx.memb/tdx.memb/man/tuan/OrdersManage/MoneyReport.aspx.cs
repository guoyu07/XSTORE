...
namespace tdx.memb.man.Tuan.OrdersManage
{
    public partial class MoneyReport : System.Web.UI.Page
    {
        ...
        public void OrdersShow()
        {
            ...
            string strsql = "select  CONVERT(varchar(10) , 下单时间, 111 ) as 下单时间 from TM_订单表 where 订单编号 in (select 订单编号 from TM_订单支付表)   group by CONVERT(varchar(10) , 下单时间, 111 )";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    strs += "'" + dt.Rows[i]["下单时间"] + "',";
                    string sql = "select SUM(总金额) as count from TM_订单表 where 订单编号 in (select 订单编号 from TM_订单支付表) and CONVERT(varchar(10) , 下单时间, 111 )='" + dt.Rows[i]["下单时间"] + "'";
        ...
        protected void sousuo_Click(object sender, EventArgs e)
        {
            ...
            {
                string sqlname = "select  CONVERT(varchar(10) , 下单时间, 111 ) as 下单时间  from TM_订单表 where 订单编号 in (select 订单编号 from TM_订单支付表) and  商品编号 in(";
              
                sqlname += "select 编号 from dbo.TM_商品表 where  用户ID in (";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        strs += "'" + dt.Rows[i]["下单时间"] + "',";
                        string sql = "select count(*) as count from dbo.TM_订单表 where CONVERT(varchar(10) , 下单时间, 111 )='" + dt.Rows[i]["下单时间"] + "'";
                        DataTable dts = DbHelperSQL.Query(sql).Tables[0];
                        if (dts.Rows.Count > 0)
                        {
                            str += dts.Rows[0]["count"] + ",";
            ...
            else
            {
                ...
                string strsql = "select  CONVERT(varchar(10) , 下单时间, 111 ) as 下单时间  from TM_订单表 where 订单编号 in (select 订单编号 from TM_订单支付表) and CONVERT(varchar(10) , 下单时间, 111 ) between '" + Jxl.Value.ToString().Replace("-", "/") + "' and '" + Jx2.Value.ToString().Replace("-", "/") + "'  group by CONVERT(varchar(10) , 下单时间, 111 )";
