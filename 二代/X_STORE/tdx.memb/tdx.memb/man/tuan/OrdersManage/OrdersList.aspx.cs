...
namespace tdx.memb.man.Tuan.OrdersManage
{
    public partial class OrdersList : System.Web.UI.Page
    {
        ...
        protected string ClassList(string _where, int page)
        {
            ...
            sql += "   dbo.WP_地址表 as dz   on d.订单编号=dz.订单编号 left join   ";
            sql += " dbo.WP_订单地址表 as dddz  on  dddz.id=dz.地址id  ";
            sql += "  where 商品编号 in(select 编号 from dbo.TM_商品表 " + _where + " )) as aa where id not in (select top (10*" + page + ") id  from (   select wd.订单编号,wd.总金额,wd.下单时间,wd.is_del,wd.openid, d.id as id, 商品编号,  价格,数量,  推荐人openID,推荐人订单号      ";
            sql += "  , 省,市,区,商圈,详细地址,手机号,收货人, 支付方式,支付金额, 支付时间,wd.备注    ";
            sql += " from  dbo.TM_订单表  as wd left join  dbo.TM_订单子表 as d on wd.订单编号=d.订单编号 left join    ";
            sql += "   dbo.TM_订单支付表 as dd on d.订单编号=dd.订单编号 left join    ";
            sql += "   dbo.WP_地址表 as dz   on d.订单编号=dz.订单编号 left join   ";
            sql += " dbo.WP_订单地址表 as dddz  on  dddz.id=dz.地址id";
            sql += "   where 商品编号 in(select 编号 from dbo.TM_商品表 " + _where + "  )) as aa  order by id desc)  order by id desc";
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < Rp_UserInfo.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    int count = comfun.DelbySQL("delete from TM_订单子表 where id=" + id);
        ...
        public void sousuo(string where)
        {
            ...
            sql += " where 商品编号 in(select 编号 from dbo.TM_商品表 )) as aa  where 1=1 " + where + " order by 下单时间 desc";
        ...
        {
            List<DTcms.Model.TM_订单支付表> listpay = new DTcms.BLL.TM_订单支付表().GetModelList(" 订单编号='" + order_no + "' and openid='" + openid + "'");
            string ispay = "未支付";
