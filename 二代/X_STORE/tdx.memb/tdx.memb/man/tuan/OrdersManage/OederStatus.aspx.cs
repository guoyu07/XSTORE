    ...
    public partial class OederStatus : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            订单编号 = Request.QueryString["id"] != null ? Request.QueryString["id"].ToString() : "";
                ...
                //sb.Append(" select  TM_订单表.订单编号,总金额,[state] as 状态, 省+','+市+','+区+','+商圈+','+详细地址 as 收货地址 ,手机号,收货人 ");
                //sb.Append("\r\n from  [dbo].[TM_订单表] left join WP_订单地址表 on  WP_订单地址表.订单编号= TM_订单表.订单编号 ");
                sb.Append(" \r\n select TM_订单表.订单编号,运费,总金额,应付款,[state] as 状态 ,手机号,收货人,                                                                      ");
                sb.Append(" \r\n isnull(省,'')+','+isnull(市,'')+','+isnull(区,'')+','+isnull(商圈,'')+','+isnull(详细地址,'') as 收货地址                             ");
                sb.Append(" \r\n from  TM_订单表 left  join WP_地址表 on TM_订单表.订单编号 =WP_地址表.订单编号 left  join WP_订单地址表 on WP_订单地址表.id=地址ID  where TM_订单表.订单编号='" + 订单编号 + "'  ");
        ...
        private void loadInfo()
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("\r\n select 商品id,订单编号,价格,数量 ,(select top 1 图片路径 from [dbo].[TM_商品图片表] where 商品编号=(select 编号  from TM_商品表 where TM_商品表.id=TM_订单子表.商品id) order by 序号 desc) as  商品图片    ");
                sb.Append("\r\n ,(select top 1 品名 from  TM_商品表 where TM_商品表.id=TM_订单子表.商品id)+'('+ (select top 1 规格 from  TM_商品表 where TM_商品表.id=TM_订单子表.商品id)+')' as  品名 ");
                sb.Append("\r\n ,(select top 1 编号new from  TM_商品表 where TM_商品表.id=TM_订单子表.商品id) as  编号new ");
                ...
                for (int j = 0; j < dorder.Rows.Count; j++)
                {
                    ...
                    sbhtml.Append("\r\n      <td>                                                                                                                                                                                                                                                ");
                    sbhtml.Append("\r\n 		<div class='al fl'>                                                                                                                                                                                                                             ");
                    sbhtml.Append("\r\n 		<a class='flk13' target='_blank' onclick=\"window.open('/shop/chanpintuangou.aspx?gid=" + dorder.Rows[j]["商品id"] + "&WHC=123', '', 'height=543,width=376,scrollbars=yes,status=yes');\" clstag='click|keycount|orderinfo|product_name'>" + dorder.Rows[j]["品名"] + " </a>                                        ");
                    sbhtml.Append("\r\n 		          </div>                                                                                                                                                                                                                                ");
                    sbhtml.Append("\r\n        <div class='clr'></div>                                                                                                                                                                                                                           ");
