...
namespace tdx.memb.man.Tuan.OrdersManage
{
    public partial class OrdersReport : System.Web.UI.Page
    {
        ...
        {
            string strsql = "select  CONVERT(varchar(10) , 下单时间, 111 ) as 下单时间 from dbo.TM_订单表  group by CONVERT(varchar(10) , 下单时间, 111 )";
        ...
        {  ///添加  商家姓名搜索  2015.7.12
            ...
            {
                string sqlname = "select  CONVERT(varchar(10) , 下单时间, 111 ) as 下单时间 from dbo.TM_订单表 where 商品编号 in(";
                sqlname += "select 编号 from dbo.TM_商品表 where  用户ID in (select id  from  dt_manager where user_name='" + txt_name.Value.Trim() + "'))  group by CONVERT(varchar(10) , 下单时间, 111 ) ";
            ...
            else
            {
                ...
                string strsql = "select  CONVERT(varchar(10) , 下单时间, 111 ) as 下单时间 from dbo.TM_订单表 where CONVERT(varchar(10) , 下单时间, 111 ) between '" + Jxl.Value.ToString() + "' and '" + Jx2.Value.ToString() + "'  group by CONVERT(varchar(10) , 下单时间, 111 )";
