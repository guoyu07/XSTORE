...
namespace tdx.memb.man.Tuan.FinanceManage
{
    public partial class FinanceList : DTcms.Web.UI.ManagePage
    {
        DTcms.BLL.WP_返利表 flbll = new DTcms.BLL.WP_返利表();
        ...
        protected string ClassList(string _where, int page)
        {
            ...
            string sql = " select top 10*  from dbo.WP_返利表 where id not in (select top (10*" + page + ") id  from dbo.WP_返利表 where  订单号 in ( ";
            sql += "   select 订单编号 from dbo.TM_订单子表  where 商品编号 in( ";
            sql += " select 编号 from dbo.TM_商品表  " + _where + " ))) and  订单号 in (   ";
            sql += "  select 订单编号 from dbo.TM_订单子表  where 商品编号 in(  ";
        ...
        private void goods(string where)
        {
            ...
            string sql = "select count(*) from dbo.WP_返利表 where  订单号 in (  ";
            sql += "select 订单编号 from dbo.TM_订单子表  where 商品编号 in(";
            ...
            if (dtid > 0)
            {
                ...
                if (dtdt.Rows.Count > 0)
                {
                    for (int i = 0; i < dtdt.Rows.Count; i++)
                    {
                        if (dtdt.Rows[i]["user_name"].ToString() != "admin")
                        {
                            string sql = "select * from dbo.WP_返利表 where  订单号 in (  ";
                            sql += "select 订单编号 from dbo.TM_订单子表  where 商品编号 in(";
                            ...
                            if (dt.Rows.Count > 0)
                            {
                                DTcms.Common.Excel.DataTable4Excel(dt, "返利Excel表");
                            }
