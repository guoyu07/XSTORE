...
namespace DTcms.Common
{
    public class Utils
    {
        ...
        #region 加载图片
        public static string GetGPICTURE(object path)
        {
            string result = string.Empty;
            if (string.IsNullOrEmpty(path.ToString()))
                result = GetGPICTUREPath() + "default.jpg";
            else
                result = path.ToString();
            return result;
        }

        public static string GetGPICTUREPath()
        {
            string result = "~/GoodsImages/";
            try
            {
                result = ConfigurationManager.AppSettings["GetGPICTUREPath"].ToString();
            }
            catch
            {
                //log error
