...
namespace DTcms.Common
{
    ...
    public static class PagingHelper
    {
        ...
        public static string CreatePagingSql(int _pageSize, int _pageIndex, string _safeSql, string _orderField)
        {
            ...
            sql += "  SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY " + _orderField + " ) as row_number,new_tb.* from (" + _safeSql + ") new_tb)AS T  WHERE row_number between " + ((Utils.StrToInt(_pageIndex.ToString(), 0) - 1) * Utils.ObjToInt(_pageSize.ToString(), 10) + 1) + " and " + Utils.StrToInt(_pageIndex.ToString(), 0) * Utils.ObjToInt(_pageSize, 10) + "";
