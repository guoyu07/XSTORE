...
namespace Wx_NewWeb.appv
{
    ...
    public   class prolist : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            string _sql = "with c as (select row_number() over(order by g_sort,id desc) as rown,*,(select c_name from b2c_category where c_no=cno ) as cname from b2c_goods where cno like '%" + _cno + "%' and g_isactive=1 and g_isdel=0 ) select top " + pagesize.ToString() + " * from c where rown > " + (Convert.ToInt32(_page) * pagesize).ToString() + " order by rown";
