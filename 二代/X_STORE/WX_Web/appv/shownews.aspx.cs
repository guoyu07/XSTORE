        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                else
                {
                    ...
                    string sql = "select * from b2c_tmsg where id='" + id + "'";
                    DataSet ds = comfun.GetDataSetBySQL(sql);
                    if (ds.Tables[0].Rows.Count > 0)
                    {
                        lt_title.Text = ds.Tables[0].Rows[0]["t_title"].ToString();

                        lt_proTitle.Text = ds.Tables[0].Rows[0]["t_title"].ToString();//bt.cname;
                        lt_proAuthor.Text = "";
                        lt_proDate.Text = ds.Tables[0].Rows[0]["t_wdate"].ToString() != null ? ("发布:" + ds.Tables[0].Rows[0]["t_wdate"].ToString()) : "";

                        lt_newsContent.Text = ds.Tables[0].Rows[0]["t_msg"].ToString();
                    }
                }
