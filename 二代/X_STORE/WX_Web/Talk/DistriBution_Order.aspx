<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="DistriBution_Order.aspx.cs" Inherits="Wx_NewWeb.Talk.DistriBution_Order" %>
...
<head>
    ...
    <script type="text/javascript">
        ...
        function loadmore() {
            var mymid = $("#hide_mid").val();
            var mypageindex = $("#pageindex").val();
            $.ajax({
                type: "POST", //用POST方式传输
                url: 'tools/LoadMoreOrder.ashx', //目标地址
                dataType: "json",//需要限定为json
                data: { mid: mymid, pageindex: mypageindex },
                success: function (data) {
                    $("#pageindex").val(data.pageindex);//改变值
                    $(".dd_list").append(data.result);
...
<body>
    <form runat="server">
        ...
        <!--订单列表开始-->
        <div class="dd_table wrap">
        	<ul class="dd_list">
                  <asp:Repeater ID="Rp_UserInfo" runat="server">
                     <ItemTemplate>
            	        <li>
                	        <div class="proj clear"><%#Eval("orderNo")%></div>
                            <div class="t_yj clear">
                    	        <div class="fl"><%#Eval("下单时间")%></div>
                                <div class="fr"><span class="price"><em>¥</em><%#Eval("总金额")%></span></div>
                            </div>
                            <div class="yj_con"><%#Eval("wx昵称")%></div>
                        </li>
                      </ItemTemplate>
                  </asp:Repeater>
            </ul>
        </div>    
            <asp:HiddenField ID="hide_mid" runat="server" />
        ...
        <!--正文列表结束-->
        <div class="more bot_80"><a href="#" id="loadmore" onclick="loadmore()">加载更多</a></div>
        <!--正文结束-->
