...
$(function() {
    var i = 0;
    $(".i_content ul").before("<div id='wap'></div>");
    $("#wap").html("<ul>" + $(".i_content ul").html() + "</ul>");
    $(".i_content ul").hide();
    $("#wap").css({
        "clear": "none",
        "width": ""
    ...
    $('#i_content_right').click(function() {
        var w = $("#wap").width();
        w_l = parseInt($("#wap").scrollLeft());

        lis_w = 0;
        $("#wap ul li").each(function() {
            lis_w += $(this).width();
		    lis_w += parseInt($(this).css("margin-right"));
        });
        //计算页码
        var page_ = (lis_w % w) == 0 ? 0 : 1;
		var pageCount= parseInt((lis_w / w) + page_);
        var page_max = pageCount * w;
        w_l += w;
        if (w_l < page_max) {
            $("#wap").scrollLeft(w_l);
        }
