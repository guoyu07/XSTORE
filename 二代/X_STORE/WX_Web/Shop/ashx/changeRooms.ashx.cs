...
namespace Wx_NewWeb.Shop.ashx
{
    ...
    /// changeRooms 的摘要说明
    /// </summary>
    public class changeRooms : IHttpHandler
    {
...
where  a.id='" + dt_hotel.Rows[0]["仓库id"].ToString() + "' and 库位名 not like '%总台%'and b.isshow=1 and (d.操作日期 is null or DateDiff(dd,d.操作日期,getdate())>=30 ) and 箱子mac is not null group by  B.id,b.库位名 ";
                   ...
                   // return JsonConvert.SerializeObject(new { state = 1, info = "lalala", data = list });
