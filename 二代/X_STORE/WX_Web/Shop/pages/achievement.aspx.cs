...
namespace Wx_NewWeb.Shop.pages
{
    public partial class achievement : BasePage
    {
        ...
        protected void PageInit()
        {
            ...
            Today.DataSource = FindDt("CONVERT(varchar(7),WP_出库表.操作日期,120) = CONVERT(varchar(7),dateadd(mm,-1,getdate()),120)", HotelInfo["id"].ObjToInt(0),out Atoatal);
            Today.DataBind();
            FindState(HotelId, "CONVERT(varchar(10),下单时间,120) = CONVERT(varchar(10),dateadd(DD,-1,getdate()),120)", "1", out daysale);
            Yestoday.DataSource = FindDt("CONVERT(varchar(10),WP_出库表.操作日期,120) = CONVERT(varchar(10),dateadd(DD,-1,getdate()),120)", HotelInfo["id"].ObjToInt(0), out Ftotal);
            Yestoday.DataBind();
            var weekday = day > 7 ? 7 : day;
            FindState(HotelId, "DateDiff(dd,下单时间,getDate())<=7", weekday.ObjToStr(), out weeksale);
            AWeek.DataSource = FindDt("DateDiff(dd,操作日期,getDate())<=7", HotelInfo["id"].ObjToInt(0),out Btoatal);
            AWeek.DataBind();
           //如果第一单到目前为止的天数大于当前的天数，则取当前天数，否则取实际的天数
            ...
            Amounth.DataSource = FindDt("DateDiff(mm,操作日期,getDate())<=0", HotelInfo["id"].ObjToInt(0), out Ctoatal); ;//
            Amounth.DataBind();
            ...
            Ayear.DataSource = FindDt("DateDiff(yy, 操作日期, getDate()) <= 0", HotelInfo["id"].ObjToInt(0), out Dtoatal); ;
            Ayear.DataBind();
        ...
        protected void pagesA()
        {
            ...
            string sql = @"select c.品名,c.本站价,sum(A.数量) as 总数,(C.本站价*sum(A.数量))as 总价 from WP_出库表  A
left join wp_库位表 b on a.库位id=b.id 
left join wp_商品表 c on c.id=A.商品id
where DateDiff(hh,操作日期,getDate())<=24 and b.仓库id='" + hotel_id + "' group by c.品名,c.本站价";
...
left join wp_库位表 b on a.库位id=b.id 
left join wp_商品表 c on c.id=A.商品id
where DateDiff(dd,操作日期,getDate())<=7 and b.仓库id='" + hotel_id + "' group by c.品名,c.本站价";
            DataTable dt = comfun.GetDataTableBySQL(sql);
            if (Btoatal == 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    Btoatal = Btoatal + dt.Rows[i]["总价"].ObjToDecimal(0);
                }
            }
            AWeek.DataSource = dt;
            AWeek.DataBind();
...
left join wp_库位表 b on a.库位id=b.id 
left join wp_商品表 c on c.id=A.商品id
where DateDiff(mm,操作日期,getDate())<=0  and b.仓库id='" + hotel_id + "' group by c.品名,c.本站价";
            DataTable dt = comfun.GetDataTableBySQL(sql);
            if (Ctoatal == 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    Ctoatal = Ctoatal + dt.Rows[i]["总价"].ObjToDecimal(0);
                }
            }
            Amounth.DataSource = dt;
            Amounth.DataBind();
        }
        //ayear
...
left join wp_库位表 b on a.库位id=b.id 
left join wp_商品表 c on c.id=A.商品id
where DateDiff(yy,操作日期,getDate())<=0 and b.仓库id='" + hotel_id + "' group by c.品名,c.本站价";
            DataTable dt = comfun.GetDataTableBySQL(sql);
            if (Dtoatal == 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    Dtoatal = Dtoatal + dt.Rows[i]["总价"].ObjToDecimal(0);
                }
            }
            Ayear.DataSource = dt;
            Ayear.DataBind();
...
where b.仓库id='" + hotel_id + "' group by c.品名,c.本站价";
            ...
            if (Etoatal == 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    Etoatal = Etoatal + dt.Rows[i]["总价"].ObjToDecimal(0);
                }
            }
            AllGoods.DataSource = dt;
            AllGoods.DataBind();
        }
        ...
        protected DataTable FindDt(string date_where, int hotel_id,out decimal total)
        {
            ...
            //            var sql =
            //                string.Format(
            //                    @"  select max(WP_商品表.品名) as 品名,max(WP_商品表.编码) as 编码,sum(WP_出库表.数量) as 总数, cast(round(sum(WP_出库表.总出价额),2) as numeric(20,2)) as 总价,WP_出库表.商品id from WP_出库表  
            //left join  WP_商品表 on WP_出库表.商品id = WP_商品表.id
            //where WP_出库表.库位id in(select id from WP_库位表 where 仓库id = {0} and 库位名 not like '%总台%')
            //and {1}
            //group by WP_出库表.商品id", hotel_id, date_where
            //);
...
where a.仓库id={0}  and  d.state in(2,3,5) and {1}
) e group by id
