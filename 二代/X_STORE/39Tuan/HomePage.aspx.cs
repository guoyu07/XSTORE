    ...
    public partial class HomePage : System.Web.UI.Page
    {
        ...
        string info(int leibietop, int infotop)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    r += "<div class=\"titleline\"><div class=\"wrap\"><div class=\"title\"><a href=\"ListView.aspx?lbh="+dt.Rows[i]["类别编号"].ToString()+"\">" + dt.Rows[i]["类别名"].ToString() + "</a></div>";
                    ...
                    if (dtinfo.Rows.Count > 0)
                    {
                        for (int j = 0; j < dtinfo.Rows.Count; j++)
                        {
                            ...
                            //string sqlbuy = "select * from TM_订单子表 where 订单编号 in (select 订单编号 from TM_订单支付表) and 商品编号 ='" + dtinfo.Rows[j]["编号"].ToString() + "'";
                            string sqlbuy = "select * from TM_订单子表 where 商品编号 ='" + dtinfo.Rows[j]["编号"].ToString() + "'";
