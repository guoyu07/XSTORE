        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (Request["unionid"] != null)
                {
                    ...
                    DataTable dtdd = comfun.GetDataTableBySQL("select a.订单号 as ddbh,* from WP_分享记录表 a,TM_订单表 b,TM_订单子表 d,TM_商品表 c where b.订单编号=d.订单编号 and  a.订单号=b.订单编号 and d.商品编号=c.编号 and a.openid in (select openid from WP_会员表 where unionid='" + unionid + "')  order by 分享时间 desc ");
                    if (dtdd.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtdd.Rows.Count; i++)
                        {
                            ...
                            str += "<div class=\"fr\"><span>分享：</span><strong>¥ " + dtdd.Rows[i]["分享时间"].ToString() + "</strong></div></div>";
