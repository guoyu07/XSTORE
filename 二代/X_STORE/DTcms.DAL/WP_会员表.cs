...
namespace DTcms.DAL
{
	...
	public partial class WP_会员表
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "WP_会员表"); 
		}
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from WP_会员表");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(DTcms.Model.WP_会员表 model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into WP_会员表(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(DTcms.Model.WP_会员表 model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update WP_会员表 set ");
            ...
            parameters[1].Value = model.wx头像;
            parameters[2].Value = model.手机号;
			parameters[3].Value = model.id;
            parameters[4].Value = model.email;
            parameters[5].Value = model.qq;
            parameters[6].Value = model.sex;
            parameters[7].Value = model.jifen;
			int rows=DbHelperSQL.ExecuteSql(strSql.ToString(),parameters);
			if (rows > 0)
			{
				return true;
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from WP_会员表 ");
			strSql.Append(" where id=@id ");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)			};
			parameters[0].Value = id;
		...
		/// 得到一个对象实体
		/// </summary>
		public DTcms.Model.WP_会员表 GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,openid,wx昵称,wx头像,手机号,password,email,qq,sex,jifen from WP_会员表 ");
			strSql.Append(" where id=@id ");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)			};
			parameters[0].Value = id;

			DTcms.Model.WP_会员表 model=new DTcms.Model.WP_会员表();
		...
		/// 得到一个对象实体
		/// </summary>
		public DTcms.Model.WP_会员表 DataRowToModel(DataRow row)
		{
			DTcms.Model.WP_会员表 model=new DTcms.Model.WP_会员表();
			if (row != null)
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from WP_会员表 T ");
