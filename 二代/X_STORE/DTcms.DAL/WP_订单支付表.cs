...
namespace DTcms.DAL
{
	...
	public partial class WP_订单支付表
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "WP_订单支付表"); 
		}
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from WP_订单支付表");
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(DTcms.Model.WP_订单支付表 model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into WP_订单支付表(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(DTcms.Model.WP_订单支付表 model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update WP_订单支付表 set ");
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from WP_订单支付表 ");
			strSql.Append(" where id=@id ");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)			};
			parameters[0].Value = id;
		...
		/// 得到一个对象实体
		/// </summary>
		public DTcms.Model.WP_订单支付表 GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,订单编号,支付方式,支付金额,openid,支付时间 from WP_订单支付表 ");
			strSql.Append(" where id=@id ");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)			};
			parameters[0].Value = id;

			DTcms.Model.WP_订单支付表 model=new DTcms.Model.WP_订单支付表();
		...
		/// 得到一个对象实体
		/// </summary>
		public DTcms.Model.WP_订单支付表 DataRowToModel(DataRow row)
		{
			DTcms.Model.WP_订单支付表 model=new DTcms.Model.WP_订单支付表();
			if (row != null)
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from WP_订单支付表 T ");
