...
namespace DTcms.DAL
{
	...
	public partial class TM_订单地址表
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("id", "TM_订单地址表"); 
		}
		...
		public bool Exists(int id)
		{
			...
			strSql.Append("select count(1) from TM_订单地址表");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into TM_订单地址表(");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update TM_订单地址表 set ");
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from TM_订单地址表 ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		public DTcms.Model.TM_订单地址表 GetModel(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 id,订单编号,省,市,区,商圈,详细地址,手机号,收货人,备注,是否为默认地址,is_del from TM_订单地址表 ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			DTcms.Model.TM_订单地址表 model=new DTcms.Model.TM_订单地址表();
		...
		/// 得到一个对象实体
		/// </summary>
		public DTcms.Model.TM_订单地址表 DataRowToModel(DataRow row)
		{
			DTcms.Model.TM_订单地址表 model=new DTcms.Model.TM_订单地址表();
			if (row != null)
			{
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from TM_订单地址表 T ");
